      ...
      h += `<div class="grp-match-card match-card-v3 tc-card${_fxOn?' grp-sidefx grp-sidefx--'+_fxMode:''}" style="--tc-win-rgb:${winRgb};${_fxVars}background:var(--white);border:1px solid var(--border);border-left:4px solid ${m.grpColor};margin-bottom:8px">
        ...
        <div class="grp-match-main" style="flex:1;display:flex;align-items:center;gap:10px;justify-content:center;flex-wrap:wrap">
          ...
          <div class="grp-score-col" style="text-align:center;min-width:60px">
            ${isDone?`<div class="grp-match-score" style="font-family:'Noto Sans KR',sans-serif;font-weight:900;font-size:var(--fs-lg);padding:6px 12px;background:var(--white);border-radius:10px;border:1.5px solid var(--border)">
              <span style="color:${aWin?'#16a34a':'var(--text3)'}">${aWin?'WIN':'패'}</span>
              <span style="color:var(--gray-l);font-size:var(--fs-sm);margin:0 2px">:</span>
  ...
  tms.forEach((tm, tmi) => {
    ...
    h += `<div class="rec-summary${_fxCls}" data-rec-mode="procompteam" style="border:1.5px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px;${_fxVars}">
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:10px">
        <span style="font-size:var(--fs-sm);font-weight:600;color:var(--text3);flex-shrink:0">${tm.d||'날짜 미정'}</span>
      ...
      <div style="display:flex;gap:16px;margin-bottom:8px;flex-wrap:wrap">
        <div style="font-size:var(--fs-caption)"><span style="color:${colA};font-weight:700">${tm.teamAName||'A팀'}:</span> <span style="color:var(--text3)">${(tm.teamA||[]).map(p=>`<span onclick="openPlayerModal('${escJS(p)}') " style="cursor:pointer;text-decoration:underline dotted">${p}</span>`).join(', ')||'미정'}</span></div>
        <div style="font-size:var(--fs-caption)"><span style="color:${colB};font-weight:700">${tm.teamBName||'B팀'}:</span> <span style="color:var(--text3)">${(tm.teamB||[]).map(p=>`<span onclick="openPlayerModal('${escJS(p)}') " style="cursor:pointer;text-decoration:underline dotted">${p}</span>`).join(', ')||'미정'}</span></div>
      </div>
      ${games.length?`<div style="border-top:1px solid var(--border);padding-top:8px;margin-top:4px">
        ${games.map((g, gi) => {
          ...
          return `<div style="display:flex;align-items:center;gap:6px;padding:4px 8px;background:var(--surface);border-radius:6px;margin-bottom:3px;font-size:var(--fs-sm)">
            ...
            <span style="color:var(--gray-l);font-size:var(--fs-caption)">WIN vs</span>
...
function _tmCyclePlayer(btn, name) {
  ...
  if (nextSide==='A') {
    ...
    tag.style.cssText='padding:2px 8px;background:#2563eb;color:#fff;border-radius:10px;font-size:var(--fs-caption);font-weight:600';
    ...
    const tag=document.createElement('span');
    tag.className='_tm_draftTag'; tag.setAttribute('data-name',name);
    tag.style.cssText='padding:2px 8px;background:#dc2626;color:#fff;border-radius:10px;font-size:var(--fs-caption);font-weight:600';
...
function proCompBracket(tn) {
  ...
  const _photo = (name, isWin, isDone, col) => {
    ...
    return p&&p.photo
      ...
      :`<div ${click} style="${pointer}width:36px;height:36px;border-radius:var(--su_profile_radius,50%);background:${isLose?'#cbd5e1':col};flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:var(--fs-base);font-weight:900;color:${isLose?'#64748b':'#fff'};${ring};${isLose?'opacity:.7;':''}">${name[0]}</div>`;
  };
  const _info = name => {
  ...
  rounds.forEach((rnd, ri) => {
    ...
    h += `<div style="display:flex;align-items:center">
      <div style="min-width:${isLast?220:200}px;flex-shrink:0">
        <div style="text-align:center;font-size:var(--fs-sm);font-weight:900;color:#fff;margin-bottom:10px;padding:7px 10px;background:${bg};border-radius:10px;box-shadow:0 3px 8px ${col}44;letter-spacing:.5px">${lbl}</div>
    ...
    rnd.forEach((m, mi) => {
      ...
      h += `<div style="border-radius:12px;overflow:hidden;background:var(--white);box-shadow:${isDone?`0 4px 16px ${col}28,0 1px 4px rgba(0,0,0,.08)`:isLast?`0 2px 12px rgba(0,0,0,.1)`:'0 1px 6px rgba(0,0,0,.07)'};border:${isLast&&isDone?`2px solid ${col}66`:isDone?`1.5px solid ${col}44`:'1.5px solid #e2e8f0'}">
        ...
        <!-- 날짜/맵/동률 -->
        ${(m.map||m.d)?`<div style="padding:3px 12px;font-size:var(--fs-caption);font-weight:600;color:var(--text3);background:#f8fafc;border-top:1px solid #f1f5f9;display:flex;gap:8px;flex-wrap:wrap">${m.d?`<span>🗓️ ${m.d.slice(2).replace(/-/g,'.')}</span>`:''}${m.map?`<span>🗺️ ${m.map}</span>`:''}</div>`:''}
        ...
        ${isTieSaved?`<div style="padding:3px 12px;font-size:var(--fs-caption);font-weight:900;color:#b45309;background:#fffbeb;border-top:1px solid #f1f5f9;display:flex;gap:8px;align-items:center">
    ...
    h += `</div></div>`;
    // 라운드 간 화살표 커넥터
    if (ri < rounds.length-1) h += `<div style="width:28px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:var(--fs-lg);color:#cbd5e1;font-weight:900;align-self:center;padding-top:36px">➔</div>`;
    h += `</div>`;
  });
  h += `</div></div>`;
  // 3위전
...
function proCompTourMatchInput(tn){
  ...
  const listHTML = sorted.length
    ...
    : `<div style="margin-top:10px;font-size:var(--fs-sm);color:var(--gray-l)">등록된 기록이 없습니다.</div>`;
...
window.openPcStageBulkPasteModal = function(tnId, round){
  ...
  modal.innerHTML=`<div class="umbox" style="width:780px;max-width:97vw;max-height:92vh;overflow:auto">
    ...
    <div style="position:relative">
      <textarea id="_pcStageBulkText" oninput="pcStageBulkPreview('${tnId}','${r}')" style="width:100%;min-height:160px;font-size:var(--fs-base);border:1.5px solid #ddd6fe;border-radius:10px;padding:12px 36px 12px 12px;resize:vertical;font-family:'Noto Sans KR',monospace;line-height:1.8;box-sizing:border-box" placeholder="예)&#10;[실피드] 홍길동T (승) vs (패) 임꺽정Z&#10;또는&#10;홍길동 임꺽정 투혼&#10;또는&#10;2026-05-02	홍길동	임꺽정	투혼"></textarea>
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ${cand.map(name=>{
        ...
        return `<div style="display:flex;align-items:center;gap:10px;border:1px solid var(--border);border-radius:10px;padding:10px 12px;background:var(--surface)">
          ...
          <select data-seed-player="${name.replace(/\"/g,'&quot;')}" style="flex:1;padding:6px 10px;border-radius:10px;border:1px solid var(--border2);font-weight:800;font-size:var(--fs-sm)">
...
function proCompOpenBktBatchModal(tnId) {
  ...
  modal.innerHTML = `
    <div style="background:var(--white);border-radius:16px;padding:24px;width:400px;max-width:100%;box-shadow:0 8px 40px rgba(0,0,0,.3)">
      ...
      <textarea id="_bktBatchText" style="width:100%;height:200px;padding:12px;border-radius:10px;border:1.5px solid var(--border);font-size:var(--fs-base);margin-bottom:16px;box-sizing:border-box;resize:none" placeholder="여기에 복사해서 붙여넣으세요..."></textarea>
...
function rProAll(C, T) {
  ...
  let h = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap">
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      <span style="font-size:var(--fs-caption);font-weight:700;padding:2px 10px;border-radius:10px;background:#dbeafe;color:#2563eb">일반 ${proM.length}경기</span>
      <span style="font-size:var(--fs-caption);font-weight:700;padding:2px 10px;border-radius:10px;background:#f3e8ff;color:#7c3aed">대회 ${compCnt}경기</span>
      <span style="font-size:var(--fs-caption);font-weight:700;padding:2px 10px;border-radius:10px;background:#f0fdf4;color:#16a34a">완료 ${compItems.length}경기</span>
    </div>
  </div>`;
  ...
  if (!proItems.length) {
    ...
    proItems.slice(0,50).forEach(item=>{
      ...
      if(item.type==='일반'){
        ...
        result=item.winner?`<span style="font-size:var(--fs-caption);font-weight:700;color:#16a34a;background:#dcfce7;padding:1px 8px;border-radius:10px">${item.winner==='A'?item.aLabel:item.bLabel} 승</span>`:`<span style="font-size:var(--fs-caption);color:var(--gray-l);background:var(--surface);padding:1px 8px;border-radius:10px">미정</span>`;
      }
...
function _renderProCompGrpShareCard(tnId, gi) {
  ...
  const rankRows = ranks.map((r, idx) => {
    ...
    return `<tr style="border-top:1px solid ${col}18;${idx===0?'background:'+col+'0a':''}">
      ...
      <td style="padding:6px 8px;text-align:center;font-weight:700;color:#16a34a;font-size:var(--fs-sm)">${r.w}</td>
      <td style="padding:6px 8px;text-align:center;color:#dc2626;font-size:var(--fs-sm)">${r.l}</td>
  ...
  tn.gjMatches.slice().reverse().forEach((sess, ri) => {
    ...
    h += `<div class="rec-summary${_fxCls}" data-rec-mode="procompgj" style="border:1px solid var(--border);border-radius:8px;margin-bottom:8px;overflow:hidden;${_fxVars}">
      ...
      ${(sess.games||[]).map((g,gi)=>{
        ...
        return `<tr>
          <td style="font-size:var(--fs-caption);color:var(--gray-l)">${gi+1}게임</td>
          ...
          <td style="font-size:var(--fs-caption);color:var(--gray-l)">${g.map||''}</td>
        </tr>`;
      }).join('')}
      </tbody></table>
    </div>`;
