...
function rTierTourCfg(C,T){
  ...
  let h=`${_cfgD('notice','📢 공지 관리')}
    ...
    ${notices.length===0?`<div style="padding:18px;text-align:center;color:var(--gray-l);background:var(--surface);border-radius:10px;font-size:var(--fs-base)">등록된 공지 없음</div>`:
  ...
  el.innerHTML = list.map(p=>{
    ...
    return `<div style="border-bottom:1px solid var(--border);padding:8px 10px">
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        <select onchange="setStatusIcon('${pN}',this.value);_cfgRefreshSiAssignRow('${pN}')" style="font-size:var(--fs-sm);padding:5px 8px;border:1px solid var(--border2);border-radius:10px;min-width:140px">${opts}</select>
