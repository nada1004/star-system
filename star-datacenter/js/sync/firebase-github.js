...
// ─────────────────────────────────────────────────────────────
// (요청사항) _autoSaveToGitHub — 경기기록/스트리머/대학 수정 후 GitHub 자동 저장
// - GitHub 토큰이 없거나 동기화 OFF면 조용히 스킵
// - 저장 중 중복 호출 방지(디바운스 1.5초)
// - 성공/실패 토스트 알림
// ─────────────────────────────────────────────────────────────
window._autoSaveToGitHub = (function(){
  let _debounceT = null;
  let _inFlight = false;

  return async function autoSaveToGitHub(label){
    // 토큰 없으면 스킵 (조용히)
    const token = (function(){ try{ return (localStorage.getItem('su_gh_token')||'').trim(); }catch(e){ return ''; } })();
    if(!token) return;
    ...
    _debounceT = setTimeout(async ()=>{
      if(_inFlight) return;
      _inFlight = true;
      const lbl = label || '데이터 저장';
      try{
        // fbUpdate 사용 — 현재 전역 상태를 GitHub에 push
        if(typeof window.fbUpdate === 'function'){
          // 전체 데이터 스냅샷 구성
          const patch = {};
          try{ if(typeof players !== 'undefined') patch.players = players; }catch(e){}
          try{ if(typeof univCfg !== 'undefined') patch.univCfg = univCfg; }catch(e){}
          try{ if(typeof miniM !== 'undefined') patch.miniM = miniM; }catch(e){}
          try{ if(typeof univM !== 'undefined') patch.univM = univM; }catch(e){}
          try{ if(typeof comps !== 'undefined') patch.comps = comps; }catch(e){}
          try{ if(typeof ckM !== 'undefined') patch.ckM = ckM; }catch(e){}
          try{ if(typeof proM !== 'undefined') patch.proM = proM; }catch(e){}
          try{ if(typeof tourneys !== 'undefined') patch.tourneys = tourneys; }catch(e){}
          try{ if(typeof ttM !== 'undefined') patch.ttM = ttM; }catch(e){}
          try{ if(typeof indM !== 'undefined') patch.indM = indM; }catch(e){}
          try{ if(typeof gjM !== 'undefined') patch.gjM = gjM; }catch(e){}
          try{ if(typeof maps !== 'undefined') patch.maps = maps; }catch(e){}
          try{ if(typeof tourD !== 'undefined') patch.tourD = tourD; }catch(e){}
          patch.savedAt = Date.now();
          await window.fbUpdate(patch);
          try{ if(typeof window.showToast==='function') window.showToast(`✅ GitHub 저장 완료 (${lbl})`, 2000); }catch(e){}
        }
      }catch(e){
        console.warn('[_autoSaveToGitHub] 실패:', e && e.message || e);
        // 실패해도 사용자에게는 조용히 (GitHub 토큰 문제면 설정에서 설정하도록)
      }finally{
        _inFlight = false;
      }
    }, 1500);
