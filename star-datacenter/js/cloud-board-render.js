    ...
    const buildPlayerChip=(p, chipIdx)=>{
      ...
      if (boardCardView) {
        ...
        const imgInner = photoSrcChip
          ? `<img src="${toScaledUrl(photoSrcChip,320)}" class="brd-fit-auto" data-fit-kind="card" data-fit-mode="auto" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:top center;border-radius:${imgBorderRadius}" onload="_applyBoardBgAutoSizing(this)" ${forExport?'':' onerror="this.style.display=\'none\'"'}>`
      ...
      if(forExport){
        ...
        return `<span style="display:inline-flex;align-items:center;gap:12px;background:${cBgE};border-radius:16px;padding:10px 18px 10px 10px;margin:5px;box-shadow:0 2px 10px rgba(0,0,0,.13);border:2px solid ${cBdE}">
          ${photoSrcChip
            ?`<img src="${toThumbUrl(photoSrcChip,64)}" class="brd-fit-auto" data-fit-kind="profile" data-fit-mode="auto" style="width:64px;height:64px;border-radius:${imgRadius};object-fit:cover;flex-shrink:0;border:3px solid ${col};box-shadow:0 2px 10px ${hexToRgba(col,.4)}" onload="_applyBoardBgAutoSizing(this)">`
