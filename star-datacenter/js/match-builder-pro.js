...
function rPro(C,T){
  ...
  const extra = (proSub!=='input' && proSub!=='brief' && typeof buildYearMonthFilterControls==='function')
  ...
  if(proSub==='input'&&_li){
    ...
    h+=proRankHTML();
  } else if(proSub==='brief'){
    h+=(typeof rProLeagueBriefing==='function' ? rProLeagueBriefing() : '');
  } else {
...
function buildProInputHTML(){
  ...
  let h = _mbSectionCard('① 기본 정보', `
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
      ...
      <div style="font-size:var(--fs-caption);color:var(--blue);margin-top:6px">대상 스트리머: <strong>${eligible.length}명</strong></div>
    `, '(복수 선택 · god~1티어 · 검색으로 여성 선수도 추가 가능)') + _mbSectionCard('③ 스트리머 클릭 → 팀 배정', `
      <div style="display:flex;flex-wrap:wrap;gap:6px;padding:10px;background:var(--surface);border:1px solid var(--border);border-radius:8px;max-height:220px;overflow-y:auto">
        ${eligible.length===0
          ?'<span style="color:var(--gray-l);font-size:var(--fs-sm)">티어를 선택하면 스트리머 목록이 표시됩니다</span>'
          :eligible.map(p=>{
              ...
              if(inA||inB){
                return `<span style="display:inline-flex;align-items:center;gap:3px;background:${bg};color:#fff;padding:4px 8px;border-radius:6px;font-size:var(--fs-caption);opacity:0.55">${p.name}${p.gender==='F'?'<span style="font-size:9px;color:#fda4af">♀</span>':''}<span style="opacity:.8;font-size:10px;margin-left:2px">${p.univ}/${p.tier}</span><span style="background:rgba(255,255,255,.3);border-radius:2px;padding:0 4px;font-size:9px;font-weight:800;margin-left:3px">${inA?'A팀':'B팀'}</span></span>`;
              }
    ...
    <div class="mb-split">
      <div class="ck-panel">
        ...
        <input type="text" id="pro-tla-input" value="${(bld.teamNameA||'').replace(/"/g,'&quot;')}" placeholder="팀명 (미입력 시 A팀)" style="width:100%;padding:5px 8px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm);margin-bottom:6px;font-weight:700" onchange="BLD['pro'].teamNameA=this.value">
        <div style="display:flex;gap:6px;margin-bottom:6px">
          <input type="text" id="pro-a-search" placeholder="🔍 이름·메모/별명 검색..." style="flex:1;padding:5px 8px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="proSearchPlayer('A')" onkeydown="if(event.key==='Enter')proAddByQuery('A')">
        </div>
        ...
        <div>${mA.map((m,i)=>`<span class="mem-tag" style="background:${gc(m.univ)}">${m.name}<span style="font-size:10px;opacity:.8">(${m.univ}${m.tier?'/'+m.tier:''}${m.race?'/'+m.race:''})</span><button onclick="BLD['pro'].membersA.splice(${i},1);BLD['pro'].sets=[];render()">×</button></span>`).join('')||'<span style="color:var(--gray-l);font-size:var(--fs-sm)">스트리머 없음</span>'}</div>
      </div>
      <div class="ck-panel">
        <h4>🔴 팀 B (${mB.length}명)</h4>
        <input type="text" id="pro-tlb-input" value="${(bld.teamNameB||'').replace(/"/g,'&quot;')}" placeholder="팀명 (미입력 시 B팀)" style="width:100%;padding:5px 8px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm);margin-bottom:6px;font-weight:700" onchange="BLD['pro'].teamNameB=this.value">
        <div style="display:flex;gap:6px;margin-bottom:6px">
          <input type="text" id="pro-b-search" placeholder="🔍 이름·메모/별명 검색..." style="flex:1;padding:5px 8px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="proSearchPlayer('B')" onkeydown="if(event.key==='Enter')proAddByQuery('B')">
        </div>
        ...
        <div>${mB.map((m,i)=>`<span class="mem-tag" style="background:${gc(m.univ)}">${m.name}<span style="font-size:10px;opacity:.8">(${m.univ}${m.tier?'/'+m.tier:''}${m.race?'/'+m.race:''})</span><button onclick="BLD['pro'].membersB.splice(${i},1);BLD['pro'].sets=[];render()">×</button></span>`).join('')||'<span style="color:var(--gray-l);font-size:var(--fs-sm)">스트리머 없음</span>'}</div>
      </div>
    </div>`) + _mbSectionCard('⑤ 경기 결과 입력', `
      <div style="margin-bottom:10px;padding:10px 12px;border:1px solid rgba(14,165,233,.22);background:linear-gradient(135deg,rgba(239,246,255,.96),rgba(248,250,252,.98));border-radius:var(--r);font-size:var(--fs-caption);color:#0f172a;line-height:1.6">
...
function proSearchPlayer(team){
  ...
  if(!results.length){
    dropEl.innerHTML='<div style="padding:10px;color:var(--gray-l);font-size:var(--fs-sm);text-align:center">검색 결과 없음</div>';
  ...
  dropEl.innerHTML=results.map(p=>`<div onclick="proAddPlayer('${team}','${escJS(p.name)}');document.getElementById('pro-${team.toLowerCase()}-search').value='';document.getElementById('pro-${team.toLowerCase()}-drop').style.display='none'"
    style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;font-size:var(--fs-sm)"
    onmouseover="this.style.background='var(--blue-l)'" onmouseout="this.style.background=''">
    <span style="display:inline-block;width:28px;height:28px;border-radius:6px;background:${gc(p.univ)};color:#fff;text-align:center;line-height:28px;font-size:var(--fs-caption);font-weight:700;flex-shrink:0">${(p.race||'?').charAt(0)}</span>
    <div>
...
function proTeamResultsHTML(){
  ...
  let h=`<div style="font-family:'Noto Sans KR',sans-serif;font-weight:900;font-size:var(--fs-md);color:var(--blue);margin-bottom:14px;padding-bottom:6px;border-bottom:2px solid var(--blue-ll)">⚔️ 팀전 결과 집계</div>`;
  ...
  if(pairs.length){
    h+=`<div style="margin-bottom:20px;border-radius:12px;overflow:hidden;border:1px solid var(--border)">
      <div style="padding:10px 16px;background:linear-gradient(135deg,#1e3a8a,#2563eb);color:#fff;font-weight:900;font-size:var(--fs-base)">🏆 팀 간 대결 기록</div>
      ...
      h+=`<tr style="border-top:1px solid var(--border)">
        <td style="padding:8px 12px">
          <div style="display:flex;align-items:center;gap:4px">
            ...
            <span style="font-size:var(--fs-base);font-weight:800;color:#16a34a;margin-left:4px">${aW}승</span>
          </div>
        </td>
        <td style="padding:8px 4px;text-align:center;font-weight:900;color:var(--gray-l)">vs</td>
        <td style="padding:8px 12px;text-align:right">
          <div style="display:flex;align-items:center;justify-content:flex-end;gap:4px">
            <span style="font-size:var(--fs-base);font-weight:800;color:#16a34a;margin-right:4px">${bW}승</span>
  ...
  h+=`<div style="margin-bottom:20px;border-radius:12px;overflow:hidden;border:1px solid var(--border)">
    <div style="padding:10px 16px;background:linear-gradient(135deg,#0891b2,#0e7490);color:#fff;font-weight:900;font-size:var(--fs-base)">📋 경기별 팀전 결과 (${proM.length}경기)</div>
  ...
  sorted.forEach(m=>{
    ...
    h+=`<div style="display:flex;align-items:center;gap:8px;padding:8px 14px;border-bottom:1px solid var(--border);flex-wrap:wrap">
      <span style="font-size:var(--fs-sm);font-weight:600;color:var(--text3);white-space:nowrap;min-width:80px">${m.d||'날짜미정'}</span>
      <div style="flex:1;display:flex;align-items:center;gap:6px;justify-content:center;flex-wrap:wrap">
        ...
        <div style="font-family:'Noto Sans KR',sans-serif;font-weight:900;font-size:var(--fs-md);padding:3px 10px;background:var(--surface);border-radius:8px;border:1px solid var(--border)">
          <span style="color:${aWin?'var(--win-col)':bWin?'var(--lose-col)':'var(--text3)'}">${sa}</span>
          <span style="color:var(--gray-l);font-size:var(--fs-caption);margin:0 2px">:</span>
  ...
  if(mapArr.length){
    ...
    h+=`<div style="margin-bottom:20px;border-radius:12px;overflow:hidden;border:1px solid var(--border)">
      <div style="padding:10px 16px;background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;font-weight:900;font-size:var(--fs-base)">📍 맵 사용 통계 (총 ${total}게임)</div>
      <div style="padding:12px 16px;display:flex;flex-direction:column;gap:8px">`;
    mapArr.slice(0,15).forEach(([map,s])=>{
      const pct=total?Math.round(s.total/total*100):0;
      h+=`<div style="display:flex;align-items:center;gap:10px">
        <span style="font-size:var(--fs-sm);font-weight:600;min-width:140px;color:var(--text)">📍 ${map}</span>
        ...
        <span style="font-size:var(--fs-caption);font-weight:700;min-width:50px;text-align:right">${s.total}회 (${pct}%)</span>
      </div>`;
    });
    h+=`</div></div>`;
  }
...
function proRankHTML(){
  ...
  let h=`<div style="font-family:'Noto Sans KR',sans-serif;font-weight:900;font-size:var(--fs-md);color:var(--blue);margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid var(--blue-ll)">🏅 프로 순위</div>
  ...
  _paged.forEach((p,i)=>{
    ...
    else rnkHTML=`<span style="font-family:'Noto Sans KR',sans-serif;font-weight:900;font-size:var(--fs-base)">${_ri+1}위</span>`;
    h+=`<tr>
      ...
      <td class="wt" style="font-size:var(--fs-md);font-weight:800">${p.w}</td>
      <td class="lt" style="font-size:var(--fs-md);font-weight:800">${p.l}</td>
