(function(){
  function statsCsvExportHTML(){
    ...
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec"><h4 style="margin-bottom:14px">📥 CSV / 데이터 내보내기</h4><div style="display:flex;flex-wrap:wrap;gap:10px">
      <div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:18px;flex:1;min-width:220px"><div style="font-size:20px;margin-bottom:6px">👤</div><div style="font-weight:800;font-size:14px;margin-bottom:4px">선수 전적 CSV</div><div style="font-size:11px;color:var(--gray-l);margin-bottom:12px">이름, 대학, 티어, 종족, 승, 패, 승률, ELO, 포인트</div><button class="btn btn-b btn-sm" onclick="csvDownloadPlayers()">📥 다운로드</button></div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:18px;flex:1;min-width:220px"><div style="font-size:20px;margin-bottom:6px">📋</div><div style="font-weight:800;font-size:14px;margin-bottom:4px">경기 히스토리 CSV</div><div style="font-size:11px;color:var(--gray-l);margin-bottom:12px">날짜, 승자, 패자, 맵, ELO변화, 매치ID</div><button class="btn btn-b btn-sm" onclick="csvDownloadHistory()">📥 다운로드</button></div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:18px;flex:1;min-width:220px"><div style="font-size:20px;margin-bottom:6px">🏛️</div><div style="font-weight:800;font-size:14px;margin-bottom:4px">대학별 통계 CSV</div><div style="font-size:11px;color:var(--gray-l);margin-bottom:12px">대학, 총승, 총패, 승률, 선수수</div><button class="btn btn-b btn-sm" onclick="csvDownloadUniv()">📥 다운로드</button></div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:18px;flex:1;min-width:220px"><div style="font-size:20px;margin-bottom:6px">🗺️</div><div style="font-weight:800;font-size:14px;margin-bottom:4px">맵별 통계 CSV</div><div style="font-size:11px;color:var(--gray-l);margin-bottom:12px">맵, 총경기, 승횟수, 패횟수</div><button class="btn btn-b btn-sm" onclick="csvDownloadMaps()">📥 다운로드</button></div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:12px;padding:18px;flex:1;min-width:220px"><div style="font-size:20px;margin-bottom:6px">💾</div><div style="font-weight:800;font-size:14px;margin-bottom:4px">전체 백업 JSON</div><div style="font-size:11px;color:var(--gray-l);margin-bottom:12px">모든 데이터 JSON 백업 (기존 기능)</div><button class="btn btn-w btn-sm" onclick="doExport()">📤 JSON 백업</button></div>
    </div></div></div>`;
  }
