...
function rBracketSchedule(tn){
  ...
  function matchCard(mc){
    ...
    return `<div style="margin-bottom:8px;position:relative">
      ${dateStr?`<span style="position:absolute;top:6px;left:10px;z-index:2;font-size:9px;color:var(--gray-l);font-weight:700;letter-spacing:.3px;pointer-events:none">${dateStr.slice(5).replace('-','/')}</span>`:''}
      <div class="grp-match-card match-card-v3 tc-card${_fxOn?' grp-sidefx grp-sidefx--'+_fxMode:''}${(_bktSide.left||_bktSide.right)?' has-side-panels':''}" style="--tc-win-rgb:${winRgb};${_sideRgbVars}${_fxVars}border-left:4px solid ${(!ca||ca==='#6b7280')?( isManual?'#7c3aed':'var(--blue)'):ca};border-right:4px solid ${(!cb||cb==='#6b7280')?(isManual?'#7c3aed':'var(--blue)'):cb};background:var(--white);margin-bottom:0;padding-top:${dateStr?"22px":"0"}">
