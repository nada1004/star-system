(function(){
  ...
  function statsClutchHTML(){
    ...
    function clutchRow(e,i){ const col=window.gc(e.univ); const rateCol=e.rate>=60?'#7c3aed':e.rate>=50?'var(--green)':e.rate>=40?'var(--gold)':'var(--red)'; const badge=i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}`; return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--white);border:1px solid var(--border);border-radius:8px;cursor:pointer" onclick="openPlayerModal('${escJS(e.name)}')"><span style="min-width:24px;font-size:15px">${badge}</span><span style="font-weight:800;font-size:13px;color:var(--blue);min-width:65px">${e.name}</span><span style="font-size:11px;color:${col};font-weight:700;min-width:55px">${e.univ}</span><div style="flex:1;background:var(--border2);border-radius:20px;height:10px;overflow:hidden"><div style="width:${e.rate}%;background:${rateCol};height:100%;border-radius:20px"></div></div><span style="font-weight:900;font-size:14px;color:${rateCol};min-width:40px;text-align:right">${e.rate}%</span><span style="font-size:11px;color:var(--gray-l)">${e.w}W ${e.l}L (${e.tot}경기)</span></div>`; }
  ...
  function statsStreakHistHTML(){
    ...
    function streakRow(s,i){ const col=window.gc(s.univ); const isWin=s.type==='승'; const badge=i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}`; return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--white);border:1px solid var(--border);border-radius:8px;cursor:pointer;${s.current?'border-color:'+( isWin?'#dc2626':'#2563eb')+';box-shadow:0 0 0 2px '+(isWin?'#fee2e2':'#dbeafe'):''}" onclick="openPlayerModal('${escJS(s.name)}')"><span style="min-width:24px;font-size:15px">${badge}</span><span style="font-weight:900;font-size:20px;color:${isWin?'#dc2626':'#2563eb'};min-width:48px">${s.n}</span><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:13px">${s.name} <span style="font-size:10px;color:${col};font-weight:600">${s.univ}</span>${s.current?`<span style="font-size:10px;background:${isWin?'#fee2e2':'#dbeafe'};color:${isWin?'#dc2626':'#2563eb'};padding:1px 6px;border-radius:4px;font-weight:700;margin-left:4px">진행중</span>`:''}</div><div style="font-size:10px;color:var(--gray-l)">${s.start}${s.end&&s.end!==s.start?' ~ '+s.end:''}</div></div><span style="font-weight:800;font-size:13px;color:${isWin?'#dc2626':'#2563eb'};white-space:nowrap">연${isWin?'승':'패'}</span></div>`; }
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec"><h4 style="margin-bottom:14px">🔥 역대 연속 기록 히스토리 <span style="font-size:11px;color:var(--gray-l);font-weight:400">(3연속 이상)</span></h4><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><div><div style="font-weight:800;font-size:13px;color:#dc2626;margin-bottom:8px;padding:7px 12px;background:#fef2f2;border-radius:8px;border-left:4px solid #dc2626">🏆 역대 최장 연승 TOP 15</div><div style="display:flex;flex-direction:column;gap:4px">${winStreaks.length?winStreaks.map(streakRow).join(''):'<p style="color:var(--gray-l);padding:16px;text-align:center">기록 없음</p>'}</div></div><div><div style="font-weight:800;font-size:13px;color:#2563eb;margin-bottom:8px;padding:7px 12px;background:#eff6ff;border-radius:8px;border-left:4px solid #2563eb">❄️ 역대 최장 연패 TOP 15</div><div style="display:flex;flex-direction:column;gap:4px">${loseStreaks.length?loseStreaks.map(streakRow).join(''):'<p style="color:var(--gray-l);padding:16px;text-align:center">기록 없음</p>'}</div></div></div></div></div>`;
  }
