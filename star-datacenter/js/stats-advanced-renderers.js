(function(){
  ...
  function statsRaceTrendHTML(){
    ...
    if(!months.length) return`<div class="ssec"><p style="color:var(--gray-l);padding:40px;text-align:center">경기 기록이 없습니다.<br><span style="font-size:var(--fs-caption)">경기 기록에 선수 정보와 종족이 입력되어야 집계됩니다.</span></p></div>`;
  ...
  function statsKillerHTML(){
    ...
    function calcKiller(targetName){
      ...
      const oppMap={};
      const _lastN=window._killerLastN|0;
      let _kHist=(window.statsNonProHist?window.statsNonProHist(target):_statsAllHistLocal(target));
      if(_lastN>0){
        _kHist=[..._kHist].sort((a,b)=>(a.date||'').localeCompare(b.date||'')).slice(-_lastN);
      }
      _kHist.forEach(h=>{ if(!h.opp)return; if(!oppMap[h.opp])oppMap[h.opp]={w:0,l:0}; if(h.result==='승')oppMap[h.opp].w++; else oppMap[h.opp].l++; });
    ...
    function oppRow(e,isKiller){
      ...
      const clickAttr=e.retired?'style="cursor:default"':`onclick="openPlayerModal('${escJS(e.name)}')"`;
      return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--white);border:1px solid var(--border);border-radius:8px;${e.retired?'opacity:.7':'cursor:pointer'}" ${clickAttr}>${e.retired?'':window.getPlayerPhotoHTML(e.name,'28px')}<span style="font-weight:800;font-size:var(--fs-base);color:${e.retired?'var(--text3)':'var(--blue)'};min-width:65px">${e.name}${e.retired?retiredTag:window.getStatusIconHTML(e.name)}</span><span style="font-size:var(--fs-caption);color:${col};font-weight:700;min-width:55px">${e.univ||'-'}</span><div style="flex:1;background:var(--border2);border-radius:20px;height:10px;overflow:hidden"><div style="width:${myRate}%;background:${isKiller?'var(--red)':'var(--green)'};height:100%;border-radius:20px"></div></div><span style="font-weight:800;font-size:var(--fs-sm);color:${isKiller?'var(--red)':'var(--green)'};white-space:nowrap;min-width:52px">${myRate}% (${myW}W${myL}L)</span><span style="font-size:10px;color:var(--gray-l)">${e.tot}경기</span></div>`;
    }
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec"><h4 style="margin-bottom:12px">🗡️ 킬러 & 피해자 선수</h4><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px"><span style="font-size:var(--fs-sm);font-weight:600;color:var(--text3)">선수 선택:</span><select id="killer-player-select" style="padding:6px 12px;border:1.5px solid var(--border2);border-radius:8px;font-size:var(--fs-base);font-weight:600;min-width:220px" onchange="syncKillerSelection((function(v){try{var t=document.createElement('textarea');t.innerHTML=v;return t.value;}catch(e){return v;}})(this.value))">${playersWithHistory.map(p=>`<option value="${window.escHTML?window.escHTML(p.name):p.name}"${window._killerSelPlayer===p.name?' selected':''}>${p.name} (${p.univ})</option>`).join('')}</select><input list="killer-search-list" placeholder="🔍 스트리머 검색" style="padding:6px 10px;border:1.5px solid var(--border2);border-radius:8px;font-size:var(--fs-sm);width:150px" onkeydown="if(event.key==='Enter'){applyKillerSearch(this.value);}" onchange="applyKillerSearch(this.value)"><datalist id="killer-search-list">${playersWithHistory.map(p=>`<option value="${window.escHTML?window.escHTML(p.name):p.name}">`).join('')}</datalist>${target?`<span class="ubadge" style="background:${tColor}">${target.univ}</span><span style="font-size:var(--fs-sm);color:var(--gray-l)">${target.win||0}승 ${target.loss||0}패</span>`:''}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;flex-wrap:wrap"><div><div style="font-weight:800;font-size:14px;color:var(--red);margin-bottom:8px;padding:8px 12px;background:#fef2f2;border-radius:8px;border-left:4px solid var(--red)">💀 나를 가장 많이 이긴 선수 (천적)</div><div style="display:flex;flex-direction:column;gap:4px">${killers.length?killers.map(e=>oppRow(e,true)).join(''):'<p style="color:var(--gray-l);padding:16px;text-align:center">천적 없음 👑</p>'}</div></div><div><div style="font-weight:800;font-size:14px;color:var(--green);margin-bottom:8px;padding:8px 12px;background:#f0fdf4;border-radius:8px;border-left:4px solid var(--green)">🏆 내가 가장 많이 이긴 선수 (피해자)</div><div style="display:flex;flex-direction:column;gap:4px">${victims.length?victims.map(e=>oppRow(e,false)).join(''):'<p style="color:var(--gray-l);padding:16px;text-align:center">피해자 없음</p>'}</div></div></div></div></div>`;
  }
  ...
  function statsClutchHTML(){
    ...
    function clutchRow(e,i){ const col=window.gc(e.univ); const rateCol=e.rate>=60?'#7c3aed':e.rate>=50?'var(--green)':e.rate>=40?'var(--gold)':'var(--red)'; const badge=i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}`; return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--white);border:1px solid var(--border);border-radius:8px;cursor:pointer" onclick="openPlayerModal('${escJS(e.name)}')"><span style="min-width:24px;font-size:var(--fs-md)">${badge}</span><span style="font-weight:800;font-size:var(--fs-base);color:var(--blue);min-width:65px">${e.name}</span><span style="font-size:var(--fs-caption);color:${col};font-weight:700;min-width:55px">${e.univ}</span><div style="flex:1;background:var(--border2);border-radius:20px;height:10px;overflow:hidden"><div style="width:${e.rate}%;background:${rateCol};height:100%;border-radius:20px"></div></div><span style="font-weight:900;font-size:14px;color:${rateCol};min-width:40px;text-align:right">${e.rate}%</span><span style="font-size:var(--fs-caption);color:var(--gray-l)">${e.w}W ${e.l}L (${e.tot}경기)</span></div>`; }
    const totalAceGames=aceList.reduce((s,e)=>s+e.tot,0)/2;
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec"><h4 style="margin-bottom:6px">⚡ 클러치 지수 <span style="font-size:var(--fs-caption);color:var(--gray-l);font-weight:400">에이스 결정전 / 타이브레이커 승률</span></h4><p style="font-size:var(--fs-sm);color:var(--gray-l);margin-bottom:14px">총 에이스전: ${Math.round(totalAceGames)}경기 · 경기 입력 시 세트에 <b>ACE</b> 표시된 경기만 집계됩니다</p>${aceList.length===0?'<p style="color:var(--gray-l);padding:40px;text-align:center">에이스전 기록이 없습니다.<br><span style="font-size:var(--fs-caption)">경기 입력 시 세트 설정에서 ACE 세트로 지정해 주세요.</span></p>':`<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><div><div style="font-weight:800;font-size:var(--fs-base);color:#7c3aed;margin-bottom:8px;padding:7px 12px;background:#f5f3ff;border-radius:8px;border-left:4px solid #7c3aed">🎯 클러치 킹 TOP 10</div><div style="display:flex;flex-direction:column;gap:4px">${topClutch.slice(0,10).map(clutchRow).join('')}</div></div><div><div style="font-weight:800;font-size:var(--fs-base);color:var(--red);margin-bottom:8px;padding:7px 12px;background:#fef2f2;border-radius:8px;border-left:4px solid var(--red)">💧 에이스전 약자 TOP 10</div><div style="display:flex;flex-direction:column;gap:4px">${worstClutch.map(clutchRow).join('')}</div></div></div>`}</div></div>`;
  }
  ...
  function statsStreakHistHTML(){
    ...
    function streakRow(s,i){ const col=window.gc(s.univ); const isWin=s.type==='승'; const badge=i===0?'🥇':i===1?'🥈':i===2?'🥉':`${i+1}`; return`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--white);border:1px solid var(--border);border-radius:8px;cursor:pointer;${s.current?'border-color:'+( isWin?'#dc2626':'#2563eb')+';box-shadow:0 0 0 2px '+(isWin?'#fee2e2':'#dbeafe'):''}" onclick="openPlayerModal('${escJS(s.name)}')"><span style="min-width:24px;font-size:var(--fs-md)">${badge}</span><span style="font-weight:900;font-size:20px;color:${isWin?'#dc2626':'#2563eb'};min-width:48px">${s.n}</span><div style="flex:1;min-width:0"><div style="font-weight:800;font-size:var(--fs-base)">${s.name} <span style="font-size:10px;color:${col};font-weight:600">${s.univ}</span>${s.current?`<span style="font-size:10px;background:${isWin?'#fee2e2':'#dbeafe'};color:${isWin?'#dc2626':'#2563eb'};padding:1px 6px;border-radius:4px;font-weight:700;margin-left:4px">진행중</span>`:''}</div><div style="font-size:10px;color:var(--gray-l)">${s.start}${s.end&&s.end!==s.start?' ~ '+s.end:''}</div></div><span style="font-weight:800;font-size:var(--fs-base);color:${isWin?'#dc2626':'#2563eb'};white-space:nowrap">연${isWin?'승':'패'}</span></div>`; }
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec"><h4 style="margin-bottom:14px">🔥 역대 연속 기록 히스토리 <span style="font-size:var(--fs-caption);color:var(--gray-l);font-weight:400">(3연속 이상)</span></h4><div style="display:grid;grid-template-columns:1fr 1fr;gap:16px"><div><div style="font-weight:800;font-size:var(--fs-base);color:#dc2626;margin-bottom:8px;padding:7px 12px;background:#fef2f2;border-radius:8px;border-left:4px solid #dc2626">🏆 역대 최장 연승 TOP 15</div><div style="display:flex;flex-direction:column;gap:4px">${winStreaks.length?winStreaks.map(streakRow).join(''):'<p style="color:var(--gray-l);padding:16px;text-align:center">기록 없음</p>'}</div></div><div><div style="font-weight:800;font-size:var(--fs-base);color:#2563eb;margin-bottom:8px;padding:7px 12px;background:#eff6ff;border-radius:8px;border-left:4px solid #2563eb">❄️ 역대 최장 연패 TOP 15</div><div style="display:flex;flex-direction:column;gap:4px">${loseStreaks.length?loseStreaks.map(streakRow).join(''):'<p style="color:var(--gray-l);padding:16px;text-align:center">기록 없음</p>'}</div></div></div></div></div>`;
  }
  ...
  function statsTierMatchHTML(){
    ...
    if(!activeTiers.length)return`<div class="ssec"><p style="color:var(--gray-l);padding:40px;text-align:center">티어 간 경기 기록이 없습니다.<br><span style="font-size:var(--fs-caption)">선수에 티어가 설정되고 경기 기록이 있어야 합니다.</span></p></div>`;
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec"><h4 style="margin-bottom:6px">🎖️ 티어 간 상대전적 매트릭스 <span style="font-size:var(--fs-caption);color:var(--gray-l);font-weight:400">전 경기 포함 (프로리그·남자 포함)</span></h4><p style="font-size:var(--fs-caption);color:var(--gray-l);margin-bottom:12px">가로=내 티어 / 세로=상대 티어 / 녹색=우세 빨강=열세</p><div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:var(--fs-sm);min-width:${100+activeTiers.length*72}px"><thead><tr><th style="padding:8px 12px;background:var(--surface);border:1px solid var(--border);white-space:nowrap">내↓ / 상대→</th>${activeTiers.map(t=>`<th style="padding:6px 8px;background:${tierColor[t]}22;border:1px solid var(--border);white-space:nowrap;min-width:72px"><span style="background:${tierColor[t]};color:${typeof window.getTierBtnTextColor==='function'?window.getTierBtnTextColor(t):'#fff'};padding:2px 7px;border-radius:4px;font-size:var(--fs-caption);font-weight:700">${window.getTierLabel(t)}</span></th>`).join('')}<th style="padding:6px 8px;background:var(--surface);border:1px solid var(--border);white-space:nowrap">전체</th></tr></thead><tbody>${activeTiers.map(t1=>{ const ov=tierOverview.find(x=>x.tier===t1); return`<tr><td style="padding:6px 10px;background:${tierColor[t1]}22;border:1px solid var(--border);font-weight:700;white-space:nowrap"><span style="background:${tierColor[t1]};color:${typeof window.getTierBtnTextColor==='function'?window.getTierBtnTextColor(t1):'#fff'};padding:2px 7px;border-radius:4px;font-size:var(--fs-caption)">${window.getTierLabel(t1)}</span></td>${activeTiers.map(t2=>{ if(t1===t2){ const s=matrix[t1][t2];const tot=s.w+s.l; const r=tot?Math.round(s.w/tot*100):null; return`<td style="padding:6px 8px;border:1px solid var(--border);text-align:center;background:var(--border2)"><div style="font-size:9px;color:var(--gray-l);font-weight:600">동티어</div>${tot?`<div style="font-weight:800;font-size:var(--fs-sm);color:${r>=50?'var(--green)':'var(--red)'}">${r}%</div><div style="font-size:10px;color:var(--gray-l)">${s.w}W${s.l}L</div>`:'<div style="color:var(--gray-l);font-size:var(--fs-caption)">-</div>'}</td>`; } const s=matrix[t1][t2];const tot=s.w+s.l; const r=tot?Math.round(s.w/tot*100):null; const cs=cellStyle(s.w,s.l); return`<td style="padding:5px 6px;border:1px solid var(--border);text-align:center;${cs}">${tot?`<div style="font-weight:800;font-size:var(--fs-sm)">${r}%</div><div style="font-size:10px;opacity:.7">${s.w}W${s.l}L</div>`:'<span style="color:#94a3b8;font-size:var(--fs-caption)">-</span>'}</td>`; }).join('')}<td style="padding:5px 8px;border:1px solid var(--border);text-align:center;background:var(--surface)">${ov&&ov.rate!==null?`<div style="font-weight:800;font-size:var(--fs-sm);color:${ov.rate>=50?'var(--green)':'var(--red)'}">${ov.rate}%</div><div style="font-size:10px;color:var(--gray-l)">${ov.w}W${ov.l}L</div>`:'<span style="color:var(--gray-l)">-</span>'}</td></tr>`; }).join('')}</tbody></table></div></div><div class="ssec"><h4 style="margin-bottom:12px">📊 티어별 전체 승률 요약</h4><div style="display:flex;flex-wrap:wrap;gap:10px">${tierOverview.map(t=>{ const col=tierColor[t.tier]||'#6b7280'; const tot=t.w+t.l; return`<div style="flex:1;min-width:90px;background:var(--white);border:2px solid ${col}33;border-radius:12px;padding:12px;text-align:center"><div style="background:${col};color:${typeof window.getTierBtnTextColor==='function'?window.getTierBtnTextColor(t.tier):'#fff'};padding:2px 9px;border-radius:5px;font-weight:800;font-size:var(--fs-sm);display:inline-block;margin-bottom:6px">${window.getTierLabel(t.tier)}</div><div style="font-size:24px;font-weight:900;color:${col}">${t.rate!==null?t.rate+'%':'-'}</div><div style="font-size:10px;color:var(--gray-l);margin-top:3px">${t.w}승 ${t.l}패</div>${tot?`<div style="height:4px;background:var(--border2);border-radius:2px;overflow:hidden;margin-top:5px"><div style="width:${t.rate||0}%;background:${col};height:100%;border-radius:2px"></div></div>`:''}</div>`; }).join('')}</div></div></div>`;
  }
  ...
  function statsUnivMatrix2HTML(){
    ...
    if(window._matrix2Sel.a&&window._matrix2Sel.b&&window._matrix2Sel.a!==window._matrix2Sel.b){
      ...
      detailHTML=`<div class="ssec" style="margin-top:0"><div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:14px"><select style="padding:6px 10px;border:1.5px solid ${colA};border-radius:8px;font-size:var(--fs-base);font-weight:700;color:${colA}" onchange="window._matrix2Sel.a=(function(v){try{var t=document.createElement('textarea');t.innerHTML=v;return t.value;}catch(e){return v;}})(this.value);render()">${univRank.map(u=>`<option value="${window.escHTML?window.escHTML(u.name):u.name}"${window._matrix2Sel.a===u.name?' selected':''}>${u.name}</option>`).join('')}</select><span style="font-weight:900;font-size:var(--fs-lg);color:var(--gray-l)">vs</span><select style="padding:6px 10px;border:1.5px solid ${colB};border-radius:8px;font-size:var(--fs-base);font-weight:700;color:${colB}" onchange="window._matrix2Sel.b=(function(v){try{var t=document.createElement('textarea');t.innerHTML=v;return t.value;}catch(e){return v;}})(this.value);render()">${univRank.map(u=>`<option value="${window.escHTML?window.escHTML(u.name):u.name}"${window._matrix2Sel.b===u.name?' selected':''}>${u.name}</option>`).join('')}</select><div style="margin-left:auto;display:flex;gap:12px;align-items:center"><span style="background:${colA};color:#fff;padding:4px 14px;border-radius:8px;font-weight:800">${window._matrix2Sel.a} ${sAB.w}W ${sAB.l}L</span><span style="font-weight:900;font-size:var(--fs-md)">vs</span><span style="background:${colB};color:#fff;padding:4px 14px;border-radius:8px;font-weight:800">${window._matrix2Sel.b} ${sBA.w}W ${sBA.l}L</span></div></div><div style="overflow-x:auto"><table><thead><tr><th style="background:${colA}22">${window._matrix2Sel.a} 선수</th><th style="background:${colA}22">승</th><th style="background:${colA}22">패</th><th style="background:${colA}22">승률</th><th>vs 상대 선수 (${window._matrix2Sel.b})</th></tr></thead><tbody>${aPlayers.map(p=>{ const oppDetail=Object.entries(p.opps).map(([oName,s])=>`<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 8px;background:${s.w>s.l?'#dcfce7':s.w<s.l?'#fee2e2':'#f1f5f9'};border-radius:6px;font-size:var(--fs-caption);cursor:pointer;margin:1px" onclick="openPlayerModal('${escJS(oName)}')"><b>${oName}</b> ${s.w}W${s.l}L</span>`).join(''); return`<tr><td style="font-weight:700;cursor:pointer;color:var(--blue)" onclick="openPlayerModal('${escJS(p.name)}')">${p.name}</td><td class="wt">${p.w}</td><td class="lt">${p.l}</td><td style="font-weight:800;color:${p.rate>=50?'var(--green)':'var(--red)'}">${p.tot?p.rate+'%':'-'}</td><td>${oppDetail||'<span style="color:var(--gray-l);font-size:var(--fs-caption)">-</span>'}</td></tr>`; }).join('')}</tbody></table></div></div>`;
    }
    ...
    return`<div style="display:flex;flex-direction:column;gap:14px"><div class="ssec" id="stats-univmatrix2-sec"><h4 style="margin-bottom:12px">🏛️ 대학 간 상대전적 매트릭스 (상세)</h4><div style="overflow-x:auto"><table style="border-collapse:collapse;font-size:var(--fs-sm);min-width:${60+univRank.length*72}px"><thead><tr><th style="padding:6px 10px;background:var(--surface);border:1px solid var(--border)">↓나 / 상대→</th>${univRank.map(u=>`<th style="padding:6px 8px;background:${window.gc(u.name)}22;border:1px solid var(--border);white-space:nowrap;min-width:72px"><div style="display:flex;flex-direction:column;align-items:center;gap:2px"><span style="background:${window.gc(u.name)};color:#fff;padding:2px 7px;border-radius:4px;font-size:var(--fs-caption);font-weight:700">${u.name}</span><span style="font-size:10px;color:${u.rate>=50?'var(--green)':'var(--red)'};font-weight:700">${u.rate}%</span></div></th>`).join('')}</tr></thead><tbody>${univRank.map(u=>`<tr><td style="padding:6px 10px;background:${window.gc(u.name)}22;border:1px solid var(--border);font-weight:700;white-space:nowrap"><span style="background:${window.gc(u.name)};color:#fff;padding:2px 7px;border-radius:4px;font-size:var(--fs-caption)">${u.name}</span></td>${univRank.map(v=>{ if(u.name===v.name)return`<td style="background:var(--border2);border:1px solid var(--border);text-align:center;color:var(--gray-l)">-</td>`; const s=matrix[u.name][v.name];const t=s.w+s.l;if(!t)return`<td style="background:#f8fafc;border:1px solid var(--border);text-align:center;color:var(--gray-l);font-size:var(--fs-caption)">-</td>`; const r=Math.round(s.w/t*100); return`<td style="background:${cellBg(s.w,s.l)};border:1px solid var(--border);text-align:center;padding:5px 4px;cursor:pointer" onclick="window._matrix2Sel.a='${escJS(u.name)}';window._matrix2Sel.b='${escJS(v.name)}';render();setTimeout(()=>{const el=document.getElementById('matrix2-detail');if(el)el.scrollIntoView({behavior:'smooth'});},0)"><div style="font-weight:800;font-size:var(--fs-base);color:${r>=50?'#16a34a':'#dc2626'}">${r}%</div><div style="font-size:10px;color:var(--gray-l)">${s.w}W${s.l}L</div></td>`; }).join('')}</tr>`).join('')}</tbody></table></div><p style="font-size:var(--fs-caption);color:var(--gray-l);margin-top:6px">셀 클릭 시 아래 선수별 상세 표시</p></div><div id="matrix2-detail">${detailHTML}</div></div>`;
  }
