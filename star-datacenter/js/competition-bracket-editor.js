...
function openBracketMatchModal(tnId,rnd,mi,teamA,teamB){
  ...
  document.getElementById('grpMatchBody').innerHTML=`
    <div style="background:var(--blue-l);border:1px solid var(--blue-ll);border-radius:var(--r);padding:14px;margin-bottom:16px">
      ${isManual?`<div style="margin-bottom:10px">
        <div style="font-size:var(--fs-caption);font-weight:700;color:var(--blue);margin-bottom:4px">⚔️ 라운드 (예: 8강, 준결승, 결승)</div>
        <input type="text" id="gm-rndlabel" value="${m.rndLabel||''}" placeholder="라운드명 입력 (예: 준결승)" style="width:100%;padding:6px 10px;border:1px solid var(--border2);border-radius:7px;font-size:var(--fs-base)">
      </div>`:''}
      <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start">
        <div style="flex:1;min-width:130px">
          <div style="font-size:var(--fs-caption);font-weight:700;color:var(--blue);margin-bottom:4px">🔵 팀 A</div>
        ...
        <div style="flex:1;min-width:130px">
          <div style="font-size:var(--fs-caption);font-weight:700;color:var(--red);margin-bottom:4px">🔴 팀 B</div>
...
function bktRefreshSets(){
  ...
  let h=`<div style="margin-bottom:10px;padding:10px 12px;border:1px solid rgba(99,102,241,.18);background:linear-gradient(135deg,rgba(238,242,255,.96),rgba(248,250,252,.98));border-radius:var(--r);font-size:var(--fs-caption);color:#0f172a;line-height:1.6">
      ...
      h+=`<div class="game-row">
        ...
        ${g._isTeam
          ? `<input data-bkt-a1="${si}-${gi2}" list="${bdlIdA}" value="${g.a1||''}" placeholder="A1" style="${_stA1};flex:1;min-width:72px;padding:4px 7px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="var v=this.value.split(' (')[0].trim();bktSetGame(${si},${gi2},'a1',v);bktSetGame(${si},${gi2},'playerA',[v,'${(g.a2||'').replace(/'/g,"\\'")}'].filter(Boolean).join(','))" onchange="var v=this.value.split(' (')[0].trim();bktSetGame(${si},${gi2},'a1',v);bktSetGame(${si},${gi2},'playerA',[v,(document.querySelector('[data-bkt-a2=&quot;${si}-${gi2}&quot;]')?.value||'').split(' (')[0].trim()].filter(Boolean).join(','));bktRefreshSets()">
             ...
             <input data-bkt-b1="${si}-${gi2}" list="${bdlIdB}" value="${g.b1||''}" placeholder="B1" style="${_stB1};flex:1;min-width:72px;padding:4px 7px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="var v=this.value.split(' (')[0].trim();bktSetGame(${si},${gi2},'b1',v);bktSetGame(${si},${gi2},'playerB',[v,'${(g.b2||'').replace(/'/g,"\\'")}'].filter(Boolean).join(','))" onchange="var v=this.value.split(' (')[0].trim();bktSetGame(${si},${gi2},'b1',v);bktSetGame(${si},${gi2},'playerB',[v,(document.querySelector('[data-bkt-b2=&quot;${si}-${gi2}&quot;]')?.value||'').split(' (')[0].trim()].filter(Boolean).join(','));bktRefreshSets()">
             <input data-bkt-b2="${si}-${gi2}" list="${bdlIdB}" value="${g.b2||''}" placeholder="B2" style="${_stB2};flex:1;min-width:72px;padding:4px 7px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="var v=this.value.split(' (')[0].trim();bktSetGame(${si},${gi2},'b2',v);bktSetGame(${si},${gi2},'playerB',[(document.querySelector('[data-bkt-b1=&quot;${si}-${gi2}&quot;]')?.value||'').split(' (')[0].trim(),v].filter(Boolean).join(','))" onchange="var v=this.value.split(' (')[0].trim();bktSetGame(${si},${gi2},'b2',v);bktSetGame(${si},${gi2},'playerB',[(document.querySelector('[data-bkt-b1=&quot;${si}-${gi2}&quot;]')?.value||'').split(' (')[0].trim(),v].filter(Boolean).join(','));bktRefreshSets()">`
          : `<input data-bkt-a1="${si}-${gi2}" list="${bdlIdA}" value="${g.playerA||''}" placeholder="A팀 스트리머 검색..." style="${_stA};flex:1;min-width:80px;padding:4px 7px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="bktSetGame(${si},${gi2},'playerA',this.value.split(' (')[0].trim())" onchange="bktSetGame(${si},${gi2},'playerA',this.value.split(' (')[0].trim());bktRefreshSets()">
             <span style="font-size:var(--fs-caption);color:var(--gray-l)">vs</span>
             <input data-bkt-b1="${si}-${gi2}" list="${bdlIdB}" value="${g.playerB||''}" placeholder="B팀 스트리머 검색..." style="${_stB};flex:1;min-width:80px;padding:4px 7px;border:1px solid var(--border2);border-radius:6px;font-size:var(--fs-sm)" oninput="bktSetGame(${si},${gi2},'playerB',this.value.split(' (')[0].trim())" onchange="bktSetGame(${si},${gi2},'playerB',this.value.split(' (')[0].trim());bktRefreshSets()">`}
