...
function _deliver(data) {
  ...
  if (typeof window.onFirebaseLoad === 'function') {
    ...
    // onFirebaseLoad 미등록 → pending에 저장 후 등록되면 전달
    _pending = data;
  }
}

// onFirebaseLoad가 나중에 등록될 때 _pending 전달
// 🔧 수정: _fbCallbackSet 제거 — 항상 최신 window.onFirebaseLoad 참조
(function pollCallback() {
  if (_pending && typeof window.onFirebaseLoad === 'function') {
    const d = _pending;
    _pending = null;
    window.onFirebaseLoad(d);
  }
  if (_pending) setTimeout(pollCallback, 100);
})();
