...
function rBoard(C,T){
  ...
  <div class="no-export brd-toolbar" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:12px 16px;background:var(--white);border:1px solid var(--border);border-radius:14px;margin-bottom:20px;box-shadow:0 2px 12px rgba(0,0,0,.07)">
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      ...
      <button class="brd-tbtn" onclick="boardCardView=!boardCardView;render()" style="${boardCardView?'background:#fdf4ff;border-color:#a855f7;color:#7e22ce;':''}" title="포토카드 뷰 전환">🖼️ ${boardCardView?'목록':'카드'}</button>
...
function buildUnivBoardCard(u, forExport){
  ...
  const buildChipLayout=(isWide)=>{
    ...
    return `<div class="brd-card" data-univ="${u.name}" style="--brd-col:${toPastel(col,Math.min(1, Math.max(0.35, 0.95 - b2BgAlpha * 0.01) + 0.08))};--brd-shd:${shd}${isWide?';grid-column:1/-1':''}" draggable="false">
