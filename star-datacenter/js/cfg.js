/* cfg.js - 설정 탭 및 공통 유틸 (tier-tour.js에서 분리) */
...
function rCfg(C,T){
  ...
  if(!isLoggedIn){
    h+='<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 20px;text-align:center;gap:16px"><div style="font-size:48px">??</div><div style="font-size:18px;font-weight:800;color:var(--text)">관리자 전용 페이지</div><div style="font-size:13px;color:var(--gray-l)">아래 설정은 관리자 로그인 후 이용할 수 있습니다.</div><button class="btn btn-b" onclick="om(\'loginModal\')">&#128273; 로그인</button></div>';
    C.innerHTML=h;
    setTimeout(_renderCfgPdSection, 10);
    return;
  }
  const typeOpts=[{v:'??',l:'?? 일반 공지'},{v:'??',l:'?? 중요'},{v:'??',l:'?? 경고/주의'},{v:'??',l:'?? 이벤트'}];
    ...
    ${notices.length===0?`<div style="padding:18px;text-align:center;color:var(--gray-l);background:var(--surface);border-radius:10px;font-size:13px">등록된 공지 없음</div>`:
      notices.map((n,i)=>`<div style="border:1px solid var(--border);border-radius:10px;padding:12px 14px;margin-bottom:8px;background:${n.active?'var(--white)':'var(--surface)'};opacity:${n.active?1:0.6}">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
          ...
          <button class="btn btn-r btn-xs" onclick="if(confirm('공지를 삭제할까요?')){notices.splice(${i},1);save();render()}">???</button>
        </div>
        ${(n.body||'').length>120
  ...
  ${(()=>{
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
      <h4 style="margin:0;color:#dc2626">?? 동명이인 감지 (${uniq.length}건)</h4>
  ...
  ${_cfgD('univ','??? 대학 관리')}
    <div style="font-size:11px;color:var(--gray-l);margin:8px 0 10px">??? 숨김 처리된 대학은 비로그인 상태에서 현황판에 표시되지 않습니다.</div>
  ${univCfg.map((u,i)=>{
    ...
    return `<div class="srow" style="background:${isHidden?'var(--surface)':'transparent'};border-radius:8px;padding:4px 6px;margin:-2px -6px;flex-wrap:wrap;gap:4px">
      ...
      ${isDissolved?`<span style="font-size:10px;background:#fef2f2;color:#dc2626;border:1px solid #fca5a5;border-radius:5px;padding:1px 6px;font-weight:700">??? 해체 ${u.dissolvedDate||''}</span>`:''}
      ...
      ${isDissolved
        ...
        : `<button class="btn btn-xs" style="background:${isHidden?'#fef2f2':'#f0fdf4'};color:${isHidden?'#dc2626':'#16a34a'};border:1px solid ${isHidden?'#fca5a5':'#86efac'};min-width:58px"
            ...
            ${isHidden?'??? 숨김':'? 표시'}</button>
          <button class="btn btn-xs" style="background:#fff7ed;color:#ea580c;border:1px solid #fed7aa" onclick="openDissolveModal(${i})">??? 해체</button>`}
      }
  ...
  ${_cfgD('maps','??? 맵 관리')}<div id="map-list">
  ...
  <div class="ssec">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
      <h4 style="margin:0">??? 스트리머 상태 아이콘 관리</h4>
  ...
  ${_cfgD('acct','?? 관리자 계정 관리')}
    ...
    <div style="margin-bottom:14px;padding:12px;background:var(--surface);border:1px solid var(--border);border-radius:8px">
      ...
      <div id="adm-list"></div>
      <button class="btn btn-r btn-xs" style="margin-top:10px" onclick="clearAllAdmins()">?? 전체 초기화 (기본값 리셋)</button>
    </div>
  ...
  <div class="ssec">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
      <h4 style="margin:0">?? Firebase 실시간 동기화</h4>
  ...
  ${_cfgD('season','?? 시즌 관리','id="cfg-season-sec"')}
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      ...
      <!-- 맵 이름 일괄 교체 -->
      <div style="padding:14px;background:var(--surface);border:1px solid var(--border);border-radius:10px">
        <div style="font-weight:700;font-size:13px;color:var(--blue);margin-bottom:10px">??? 맵 이름 일괄 교체</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:8px">
        ...
        <button class="btn btn-b btn-sm" onclick="bulkChangeMap()">??? 맵 일괄 교체</button>
      ...
      <div style="padding:14px;background:#f0f9ff;border:1px solid #bae6fd;border-radius:10px">
        <div style="font-weight:700;font-size:13px;color:#0369a1;margin-bottom:10px">??? 선수 일괄 티어 변경</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:8px">
        ...
        <button class="btn btn-b btn-sm" onclick="bulkChangeTier()">??? 티어 일괄 변경</button>
      ...
      <div style="padding:14px;background:#fff5f5;border:1px solid #fca5a5;border-radius:10px">
        <div style="font-weight:700;font-size:13px;color:#dc2626;margin-bottom:10px">??? 날짜 범위 일괄 삭제</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:8px">
        ...
        <button class="btn btn-r btn-sm" onclick="bulkDeleteByDate()">??? 범위 삭제 (되돌릴 수 없음)</button>
  ...
  <div class="ssec">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
      <h4 style="margin:0">??? 현황판 라벨 배경 이미지별 설정</h4>
    ...
    <div style="margin-bottom:14px;padding:14px;background:#f0f9ff;border:1px solid #bae6fd;border-radius:10px">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:8px">
        ...
        <button class="btn btn-r btn-sm" onclick="bulkClearBoardBgImg()">??? 전체 삭제</button>
      </div>
    </div>
...
function renderStorageInfo(){
  ...
  try{
    ...
    <div style="margin-bottom:10px">
      ...
      ${pct>=70?`<div style="font-size:11px;color:${barCol};margin-top:5px;font-weight:600">${pct>=90?'?? 저장 공간이 거의 가득 찼습니다! 데이터를 정리해 주세요.':'?? 저장 공간이 많이 사용되고 있습니다.'}</div>`:''}
    </div>
...
function renderSeasonList(){
  ...
  el.innerHTML = seasons.map((s,i) => `
    <div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--white);border:1px solid var(--border);border-radius:8px;margin-bottom:6px;flex-wrap:wrap">
      ...
      ${isLoggedIn ? '<button class="btn btn-w btn-xs" style="margin-left:auto" onclick="editSeason('+i+')">?? 수정</button><button class="btn btn-r btn-xs" onclick="deleteSeason('+i+')">???</button>' : '<span style="margin-left:auto"></span>'}
    </div>`).join('');
}
...
function bulkAddPlayers(){
  ...
  if(resultEl){
    ...
    if(skipped.length)msg+=`\n?? 중복 스킵 (${skipped.length}명): ${skipped.join(', ')}`;
...
window.openEP=function(name){
  ...
  document.getElementById('emBody').innerHTML=`
    ...
    <div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:6px">
      ${MAIN_ROLES.map(r=>{const ic=ROLE_ICONS[r]||'???';const col=ROLE_COLORS[r]||'#6b7280';return `<button type="button" onclick="const el=document.getElementById('ed-role');el.value=el.value===this.dataset.role?'':this.dataset.role;" data-role="${r}" style="padding:3px 8px;border-radius:6px;border:1.5px solid ${col};background:${p.role===r?col+'22':'var(--white)'};color:${col};font-size:11px;font-weight:700;cursor:pointer">${ic} ${r}</button>`;}).join('')}
    ...
    <div style="display:flex;gap:8px;align-items:center">
      ...
      <span id="ed-photo-preview-wrap" style="position:relative;width:40px;height:40px;border-radius:50%;overflow:hidden;flex-shrink:0;background:#e2e8f0;border:2px solid var(--border);display:${p.photo&&!p.photo.startsWith('data:')?'inline-block':'none'}">
        <img id="ed-photo-preview" src="${p.photo&&!p.photo.startsWith('data:')?p.photo:''}" style="width:40px;height:40px;object-fit:cover;display:block" onerror="this.style.display='none';const w=document.getElementById('ed-photo-warn');if(w){w.style.color='#d97706';w.textContent='?? 이미지를 불러올 수 없습니다. 다른 도메인에서 차단됐거나 URL이 잘못됐을 수 있습니다.';}">
      </span>
    </div>
...
function savePlayer(){
  ...
  if(_photo){
    ...
    if(_photo.length>2000){
      if(!confirm(`?? 사진 URL이 매우 깁니다 (${_photo.length}자).\n정상 URL인지 확인하세요. 계속 저장하시겠습니까?`)) return;
    }
  }
...
function _refreshAliasList(){
  ...
  listEl.innerHTML = entries.filter(([k])=>!k.endsWith('__disabled')).map(([k,v])=>`
    <div class="srow" style="flex-wrap:wrap">
      ...
      <button class="btn btn-r btn-xs" data-ak="${encodeURIComponent(k)}" onclick="if(confirm('약자 \''+decodeURIComponent('${encodeURIComponent(k)}')+'\'를 삭제할까요?'))delMapAlias(decodeURIComponent(this.getAttribute('data-ak')))">??? 삭제</button>
...
function _renderCfgSiList(){
  ...
  el.innerHTML=[...players].sort((a,b)=>a.name.localeCompare(b.name,'ko')).map(p=>{
    ...
    const delBtn=p.photo?`<button onclick="setProfilePhoto('${pN}','')" style="font-size:11px;padding:2px 6px;border-radius:5px;border:1px solid #fca5a5;background:#fff1f2;color:#dc2626;cursor:pointer;flex-shrink:0" title="이미지 삭제">???</button>`:'';
    ...
    return `<div style="border-bottom:1px solid var(--border)">
      ...
      <div style="display:flex;align-items:center;gap:5px;padding:0 12px 6px 52px">
        <span style="font-size:10px;color:var(--gray-l);white-space:nowrap">??? 프로필</span>
...
function saveFbPw(){
  ...
  if (!pw) { if(statusEl) statusEl.textContent = '?? 비밀번호를 입력하세요.'; return; }
...
function saveGhToken(){
  ...
  if (!val) { if(statusEl) statusEl.textContent = '?? 토큰을 입력하세요.'; return; }
