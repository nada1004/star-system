function formatUniversityInfo(univName) {
  ...
  const playerListHTML = sortedPlayers.map(p => {
    ...
    return `<div data-chatbot-quick="${escapeAttr(p.name)}" style="display:flex;align-items:center;gap:8px;padding:8px 11px;border-radius:9px;cursor:pointer;background:var(--surface);margin-bottom:4px;border:1px solid var(--border)"><span style="font-size:var(--fs-base);font-weight:700;color:var(--text);flex:1">${escapeHtml(p.name)}</span><span style="font-size:var(--fs-caption);padding:2px 7px;border-radius:6px;font-weight:700;color:${tC[0]};background:${tC[1]}">${p.tier}티어</span><span style="font-size:var(--fs-caption);color:var(--text3)">${raceIcon} ${p.race}</span></div>`;
  }).join('');

  if (logoUrl) {
    return `<div style="border-radius:14px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.13)"><div style="background:${univColor}"><div style="display:flex;align-items:center;justify-content:center;padding:20px 20px 0"><img src="${logoUrl}" style="width:100%;max-width:100%;display:block;object-fit:contain" onerror="this.style.display='none'"></div><div style="padding:12px 14px 16px;text-align:center"><div style="font-size:20px;font-weight:900;color:${textOnColor};letter-spacing:-0.3px">${escapeHtml(univName)}</div><div style="font-size:var(--fs-sm);color:${textSubOnColor};margin-top:3px">소속 선수 ${univPlayers.length}명</div></div></div><div style="background:var(--white);padding:8px 8px 4px"><div style="font-size:var(--fs-caption);font-weight:700;color:var(--text3);padding:0 3px 5px;letter-spacing:0.4px">👥 선수 목록 (티어순 · 클릭하면 조회)</div>${playerListHTML}</div></div>`;
  }
