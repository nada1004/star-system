...
function _b2UnivBlock(univName, col, members) {
  ...
  const _bgOp = uCfg.bgImgOp !== undefined ? uCfg.bgImgOp : 0.18;
  const bgImgHtml = uCfg.bgImg
    ? `<div style="position:absolute;inset:0;background:url('${uCfg.bgImg}') ${_bgPos}/${_bgSize} no-repeat;opacity:${_bgOp};pointer-events:none;z-index:0"></div>`
    : '';
