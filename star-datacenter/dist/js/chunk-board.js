...
var at=Object.defineProperty,st=Object.defineProperties;var lt=Object.getOwnPropertyDescriptors;var Ve=Object.getOwnPropertySymbols;var dt=Object.prototype.hasOwnProperty,ct=Object.prototype.propertyIsEnumerable;var Ze=(e,t,r)=>t in e?at(e,t,{enumerable:!0,configurable:!0,writable:!0,value:r}):e[t]=r,Te=(e,t)=>{for(var r in t||(t={}))dt.call(t,r)&&Ze(e,r,t[r]);if(Ve)for(var r of Ve(t))ct.call(t,r)&&Ze(e,r,t[r]);return e},Ae=(e,t)=>st(e,lt(t));function _b2UnivView(){let e=_b2VisUnivs().filter(i=>i.name!=="\uBB34\uC18C\uC18D"&&i.name);if(!e.length)return'<div style="text-align:center;color:var(--text3);padding:40px">\uD45C\uC2DC\uD560 \uB300\uD559\uC774 \uC5C6\uC2B5\uB2C8\uB2E4</div>';const t=new Set((typeof univCfg!="undefined"?univCfg:[]).filter(i=>i.dissolved).map(i=>String(i.name||"").trim())),r=new Set(e.map(i=>String(i&&i.name||"").trim()).filter(Boolean)),c={},o=[];(players||[]).forEach(i=>{const $=String((i==null?void 0:i.univ)||"").trim();r.has($)&&(i.hidden||i.retired||i.hideFromBoard||t.has($)||(o.push(i),(c[$]||(c[$]=[])).push(i)))});const b={};o.forEach(i=>{const $=i.tier||"?";b[$]=(b[$]||0)+1});const{fromN:a,toN:f}=_b2ThisWeekRange(),g=_b2WeeklyAggregate(o,String(a).replace(/(\d{4})(\d{2})(\d{2})/,"$1-$2-$3"),String(f).replace(/(\d{4})(\d{2})(\d{2})/,"$1-$2-$3")),x=g.filter(i=>i.total>0).length,n=g.reduce((i,$)=>i+$.total,0),d={P:0,T:0,Z:0};o.forEach(i=>{const $=String(i.race||"").trim().toUpperCase();($==="P"||$==="T"||$==="Z")&&d[$]++});const s=d.P+d.T+d.Z,m=s>0?["T","P","Z"].map(i=>{const $={T:{cls:"race-T",label:"\uD14C\uB780"},P:{cls:"race-P",label:"\uD504\uB85C\uD1A0\uC2A4"},Z:{cls:"race-Z",label:"\uC800\uADF8"}}[i];if(!d[i])return"";const I=Math.round(d[i]/s*100);return`<button type="button" class="b2-race-badge ${$.cls}" title="${$.label} ${I}%" style="cursor:pointer" onclick="if(typeof openB2RaceTierModal==='function')openB2RaceTierModal('${i}')">${i}<span style="opacity:.75;font-size:10px;margin-left:6px">${I}%</span></button>`}).filter(Boolean).join(""):"",T=(Array.isArray(TIERS)?TIERS:[]).filter(i=>b[i]).map(i=>{const $=typeof getTierBtnColor=="function"?getTierBtnColor(i):"#64748b",I=typeof getTierBtnTextColor=="function"&&getTierBtnTextColor(i)||"#fff";return`<button type="button" class="b2-tier-badge" title="${i}\uD2F0\uC5B4 ${b[i]}\uBA85" style="cursor:pointer;background:${$};color:${I};border:1px solid ${$}55" onclick="if(typeof openB2TierUnivModal==='function')openB2TierUnivModal('${String(i).replace(/\\/g,"\\\\").replace(/'/g,"\\'")}')">${i}</button>`}).join("");try{window._b2LastAllVisPlayers=o}catch(i){}const k=e.map(i=>{if(!(c[String(i.name||"").trim()]||[]).length)return"";const I=gc(i.name),A=_b2ContrastColor(I);return`<button class="b2-jump-chip" onclick="const el=document.getElementById('${"b2-univ-anchor-"+i.name.replace(/[^a-zA-Z0-9가-힣]/g,"_")}');if(el){el.scrollIntoView({behavior:'smooth',block:'start'});}" style="--chip-color:${I}40;border:1.5px solid ${I}bb;background:linear-gradient(135deg,${I}ee,${I}cc);color:${A}"><span style="letter-spacing:-.01em">${i.name}</span></button>`}).join(""),h=_b2GetUnivProfileViewMode(),B=(i,$)=>`
    ...
    <div style="padding:14px;border-radius:22px;border:1px solid rgba(148,163,184,.18);background:linear-gradient(180deg,rgba(255,255,255,.99),rgba(248,250,252,.96));box-shadow:0 16px 28px rgba(15,23,42,.05)">
      ...
      <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:10px;padding-top:10px;border-top:1px dashed rgba(148,163,184,.14)">
        ...
        ${B("table","\u{1F4CB} \uD14C\uC774\uBE14")}
          ...
          `,document.head.appendChild(n)}let g=document.getElementById("b2RaceTierOverlay");if(!g){g=document.createElement("div"),g.id="b2RaceTierOverlay",g.innerHTML=`
            <div class="su-modal">
          ...
          `,document.body.appendChild(g),g.addEventListener("click",d=>{try{d.target&&d.target.id==="b2RaceTierOverlay"&&window.closeB2RaceTierModal()}catch(s){}});const n=g.querySelector("#b2rtClose");n&&n.addEventListener("click",()=>window.closeB2RaceTierModal()),window.closeB2RaceTierModal=function(){const d=document.getElementById("b2RaceTierOverlay");d&&(d.style.display="none")}}window._b2RaceTierRender=function(){const n=window._b2RaceTierState||{},d=document.getElementById("b2RaceTierOverlay");if(!d)return;const s=d.querySelector("#b2rtTitle"),m=d.querySelector("#b2rtSub"),T=d.querySelector("#b2rtSummary"),k=d.querySelector("#b2rtUnivBar"),h=d.querySelector("#b2rtGroup"),B=d.querySelector("#b2rtGrid");s&&(s.textContent=`\uC885\uC871 \uBE44\uC911 \xB7 ${n.label||""}`);const M=n.univ&&n.univ!=="ALL"?(n.list||[]).filter(l=>String((l==null?void 0:l.univ)||"\uBB34\uC18C\uC18D")===n.univ):n.list||[];if(m&&(m.textContent=`${n.univ&&n.univ!=="ALL"?`${n.univ} \xB7 `:""}${M.length}\uBA85`),T){const l=Object.entries(n.counts||{}).sort((i,$)=>$[1]-i[1]||i[0].localeCompare($[0]))[0],v=Object.keys(n.univCounts||{}).length;T.innerHTML=`
              ...
              <div class="b2rt-summarycard">
                ...
                <div class="b2rt-summaryvalue">${n.univ&&n.univ!=="ALL"?"1":v}</div>
              ...
              </div>`}if(k){const l=["ALL"].concat(Object.keys(n.univCounts||{}).sort((v,i)=>(n.univCounts[i]||0)-(n.univCounts[v]||0)||v.localeCompare(i)));k.innerHTML=l.map(v=>{const i=v==="ALL"?(n.list||[]).length:(n.univCounts||{})[v]||0,$=(n.univ||"ALL")===v,I=v==="ALL"?`\uC804\uCCB4 (${i})`:`${v} (${i})`;return`<button type="button" class="b2rt-univbtn ${$?"on":""}" onclick="window._b2RaceTierSetUniv && window._b2RaceTierSetUniv('${String(v).replace(/'/g,"\\'")}')">${I}</button>`}).join("")}if(h)if((n.univ||"ALL")==="ALL"){const l=new Map;M.forEach(i=>{const $=String((i==null?void 0:i.univ)||"\uBB34\uC18C\uC18D");l.has($)?l.get($).push(i):l.set($,[i])});const v=Array.from(l.entries()).sort((i,$)=>$[1].length-i[1].length||i[0].localeCompare($[0]));h.innerHTML=`<div class="b2rt-groupgrid">${v.map(([i,$])=>{const I=typeof gc=="function"&&gc(i)||"#64748b",A=i&&i!=="\uBB34\uC18C\uC18D"&&typeof gUI=="function"?gUI(i,typeof getUnivLogoSizeStr=="function"?getUnivLogoSizeStr(i,"players","16px"):"16px"):"";return`<div class="b2rt-groupcard" onclick="window._b2RaceTierSetUniv && window._b2RaceTierSetUniv('${String(i).replace(/'/g,"\\'")}')">
          ...
          `,document.body.appendChild(a),a.addEventListener("click",m=>{try{m.target&&m.target.id==="b2TierUnivOverlay"&&window.closeB2TierUnivModal()}catch(T){}});const s=a.querySelector("#b2tuClose");s&&s.addEventListener("click",()=>window.closeB2TierUnivModal()),window.closeB2TierUnivModal=function(){const m=document.getElementById("b2TierUnivOverlay");m&&(m.style.display="none")}}window._b2TierUnivState={tier:r,pool:c,ordered:b,selectedUniv:"ALL"};const f=a.querySelector("#b2tuTitle"),g=a.querySelector("#b2tuSub"),x=a.querySelector("#b2tuBody"),n=a.querySelector("#b2tuFilter"),d=a.querySelector("#b2tuSummary");f&&(f.textContent=`\uD2F0\uC5B4 \xB7 ${r}\uD2F0\uC5B4`),window._b2TierUnivRender=function(){const s=window._b2TierUnivState||{},m=s.selectedUniv||"ALL";n&&(n.innerHTML=["ALL"].concat((s.ordered||[]).map(([k])=>k)).map(k=>{const h=k==="ALL"?(s.pool||[]).length:(((s.ordered||[]).find(([M])=>M===k)||[])[1]||[]).length;return`<button type="button" class="b2tu-filterbtn ${m===k?"on":""}" onclick="window._b2TierUnivSetFilter && window._b2TierUnivSetFilter('${String(k).replace(/'/g,"\\'")}')">${k==="ALL"?`\uC804\uCCB4 (${h})`:`${k} (${h})`}</button>`}).join(""));const T=m==="ALL"?s.pool||[]:(s.pool||[]).filter(k=>String((k==null?void 0:k.univ)||"\uBB34\uC18C\uC18D")===m);if(g&&(g.textContent=`${m!=="ALL"?`${m} \xB7 `:""}${T.length}\uBA85`),d){const k=s.ordered||[],h=k[0];d.innerHTML=`
              ...
              </div>`}if(x){const k=m==="ALL"?`<div class="b2tu-groupgrid">${(s.ordered||[]).map(([h,B])=>{const M=typeof gc=="function"&&gc(h)||"#64748b",l=h&&h!=="\uBB34\uC18C\uC18D"&&typeof gUI=="function"?gUI(h,typeof getUnivLogoSizeStr=="function"?getUnivLogoSizeStr(h,"players","16px"):"16px"):"";return`<div class="b2tu-groupcard">
    ...
    <style>
      ...
      */
      ...
      .b2-femco-tierbadge{font-size:${ne}px;padding:2px ${ae}px;border-radius:999px;border:1px solid rgba(0,0,0,.12);display:inline-flex;align-items:center;line-height:1}
      .b2-femco-role{font-size:${re}px;font-weight:1000;opacity:.9}
  ...
  `,document.body.appendChild(c)}function saveB2NewPlayer(){var m,T,k,h,B,M,l,v;const e=document.getElementById("b2-newplayer-msg"),t=(((m=document.getElementById("b2-newplayer-name"))==null?void 0:m.value)||"").trim(),r=(((T=document.getElementById("b2-newplayer-univ"))==null?void 0:T.value)||"").trim(),c=(((k=document.getElementById("b2-newplayer-role"))==null?void 0:k.value)||"").trim(),o=(((h=document.getElementById("b2-newplayer-race"))==null?void 0:h.value)||"N").trim(),b=(((B=document.getElementById("b2-newplayer-tier"))==null?void 0:B.value)||"?").trim(),a=(((M=document.getElementById("b2-newplayer-channel"))==null?void 0:M.value)||"").trim(),f=(((l=document.getElementById("b2-newplayer-photo"))==null?void 0:l.value)||"").trim(),g=(((v=document.getElementById("b2-newplayer-photo2"))==null?void 0:v.value)||"").trim();if(!t){alert("\uC774\uB984\uC740 \uD544\uC218\uC785\uB2C8\uB2E4.");return}if(players.find(i=>i.name===t)){alert("\uC774\uBBF8 \uC874\uC7AC\uD558\uB294 \uC774\uB984\uC785\uB2C8\uB2E4: "+t);return}if(f&&f.startsWith("data:")){alert("\u274C base64 \uC774\uBBF8\uC9C0(data:...)\uB294 \uC800\uC7A5/\uB3D9\uAE30\uD654\uAC00 \uC2E4\uD328\uD560 \uC218 \uC788\uC5B4 \uAE08\uC9C0\uC785\uB2C8\uB2E4. URL\uC744 \uC0AC\uC6A9\uD558\uC138\uC694.");return}const x={name:t,univ:r||"\uBB34\uC18C\uC18D",role:c||"\uD559\uC0DD",race:o,tier:b,channelUrl:a||void 0,photo:f||void 0,secondProfileFile:g||void 0};players.push(x),save(),render(),["b2-newplayer-name","b2-newplayer-channel","b2-newplayer-photo","b2-newplayer-photo2"].forEach(i=>{const $=document.getElementById(i);$&&($.value="")});const n=document.getElementById("b2-newplayer-tier");n&&(n.value="?");const d=document.getElementById("b2-newplayer-race");d&&(d.value="N");const s=document.getElementById("b2-newplayer-role");s&&(s.value="\uD559\uC0DD"),e&&(e.style.color="#16a34a",e.textContent=`\u2705 \uC800\uC7A5\uB428: ${t} (\uB2E4\uC74C \uC2A4\uD2B8\uB9AC\uBA38\uB97C \uACC4\uC18D \uB4F1\uB85D\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4)`)}async function saveB2FemcoAllImg(){return _saveB2FemcoInternal()}async function _saveB2FemcoInternal(){const t=document.querySelector('[onclick="saveB2FemcoAllImg()"]');t&&(t.disabled=!0,t.textContent="\u23F3...");try{const f="download"in document.createElement("a"),g=String(navigator.userAgent||""),x=/iPad|iPhone|iPod/i.test(g),n=/KAKAOTALK|Instagram|FBAN|FBAV|NAVER|Whale|Line/i.test(g);if(!f||x||n){const d=window.open("","_blank");if(d){try{d.document.write('<html><head><meta charset="utf-8"><title>\uC774\uBBF8\uC9C0 \uC0DD\uC131 \uC911...</title></head><body style="margin:0;font-family:sans-serif;background:#111;color:#fff;padding:14px">\uD3A8\uCF54\uC2A4\uD0C0\uC77C \uC774\uBBF8\uC9C0 \uC0DD\uC131 \uC911\uC785\uB2C8\uB2E4... \uC7A0\uC2DC\uB9CC \uAE30\uB2E4\uB824\uC8FC\uC138\uC694.</body></html>'),d.document.close()}catch(s){}window.__captureDlWin=d}}}catch(a){}const r=document.createElement("div");r.style.cssText="position:fixed;left:-9999px;top:0;padding:24px;background:#0b1220;box-sizing:border-box;",r.innerHTML=_b2FemcoView(),document.body.appendChild(r),r.querySelectorAll(".b2-femco-subnav,.b2-femco-panel,.no-export,.no-export-movebtns").forEach(a=>a.remove()),await new Promise(a=>setTimeout(a,120));try{typeof injectUnivIcons=="function"&&injectUnivIcons(r)}catch(a){console.warn("[saveB2FemcoAllImg] \uB300\uD559 \uC544\uC774\uCF58 \uC8FC\uC785 \uC2E4\uD328:",a.message)}try{typeof _imgToDataUrls=="function"&&await _imgToDataUrls(r,12e3)}catch(a){}try{typeof _waitForImages=="function"&&await _waitForImages(r,1500)}catch(a){}const c=r.scrollHeight+32,o=r.scrollWidth,b="\uD3A8\uCF54\uD604\uD669\uD310_\uC804\uCCB4_"+new Date().toISOString().slice(0,10)+".png";try{if(console.log("[\uD3A8\uCF54] \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2DC\uC791"),typeof window._captureAndSave!="function")throw new Error("\uC774\uBBF8\uC9C0 \uC800\uC7A5 \uAE30\uB2A5\uC744 \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.");await window._captureAndSave(r,o,c,b)}catch(a){console.error("[\uD3A8\uCF54\uD604\uD669 \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2E4\uD328]",a),alert(`\u274C \uD3A8\uCF54\uC2A4\uD0C0\uC77C \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2E4\uD328

`+(a.message||"\uC54C \uC218 \uC5C6\uB294 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4."))}finally{document.body.removeChild(r),t&&(t.disabled=!1,t.textContent="\u{1F4BE} \uC804\uCCB4 \uC800\uC7A5")}}function _b2UnivBlock(e,t,r,c=!1){var R;if(!e)return`<div style="border-radius:14px;border:2px dashed #ccc55;padding:16px 18px;background:#f5f5f5;display:flex;align-items:center;gap:10px;opacity:.7">
    ...
    </div>`;const o=univCfg.find(u=>u.name===e)||{},b=o.icon||o.img||UNIV_ICONS[e]||"",a=_b2ContrastColor(t),f=t+_b2AlphaHex(b2BgAlpha),g=t+_b2AlphaHex(b2LabelAlpha),x=!!o.bgImg,n="linear-gradient(180deg,rgba(255,255,255,.98),rgba(248,250,252,.94))",d=x?"rgba(255,255,255,.18)":"rgba(255,255,255,.55)",s=x?"linear-gradient(180deg,rgba(255,255,255,.00),rgba(248,250,252,.00))":n,m=x?"linear-gradient(180deg,rgba(255,255,255,.12),rgba(248,250,252,.04))":n,T=x?"rgba(255,255,255,.04)":d,k=x?"none":"0 10px 18px rgba(15,23,42,.04)";if(!r.length)return`<div style="border-radius:14px;border:2px dashed ${t}55;padding:16px 18px;background:${f};display:flex;align-items:center;gap:10px;opacity:.7">
    ...
    </div>`;const h=r.filter(u=>_B2_ROLE_ORDER.includes(u.role||""));h.sort((u,w)=>_b2RoleRank(u)-_b2RoleRank(w));const B=r.filter(u=>!_B2_ROLE_ORDER.includes(u.role||"")),M={};B.forEach(u=>{const w=u.tier||"?";M[w]||(M[w]=[]),M[w].push(u)});const l=TIERS.filter(u=>M[u]).concat(Object.keys(M).filter(u=>!TIERS.includes(u))),v=o.memo||"",i=(o.memoImgs||[]).concat(o.memoImg?[o.memoImg]:[]),$=!!(v||i.length),I=(u,w,C)=>`
    ...
    </div>`,A={},P=[];h.forEach(u=>{const w=u.role||"";A[w]||(A[w]=[],P.push(w)),A[w].push(u)});const H=o.bgImgPos||"center center",_=o.bgImgSize||"auto",L=((R=o.bgImgAlpha)!=null?R:b2BgImgAlpha)/100,j=(x?Math.max(.64,L+.08):L).toFixed(2),E=String(e||"").trim(),q=E.toUpperCase(),N=(()=>{const u={wmGlobalOn:1,wmOn:1,wmScale:150,wmRight:120,wmBottom:30,bgScale:100};try{const w=String(localStorage.getItem("su_b2_univ_logo_overlay_v1")||"").trim(),C=w?JSON.parse(w)||{}:{},V=C&&C.default&&typeof C.default=="object"?C.default:{},G=C&&C.perUniv&&typeof C.perUniv=="object"?C.perUniv:{},Z=E&&G&&G[E]&&typeof G[E]=="object"?G[E]:{},F=Object.assign({},u,V,Z);return F.wmGlobalOn=F.wmGlobalOn==null||Number(F.wmGlobalOn)?1:0,F.wmOn=F.wmOn==null||Number(F.wmOn)?1:0,F.wmScale=Math.max(50,Math.min(250,parseInt(F.wmScale||150,10)||150)),F.wmRight=Math.max(0,Math.min(260,parseInt(F.wmRight||120,10)||120)),F.wmBottom=Math.max(0,Math.min(160,parseInt(F.wmBottom||30,10)||30)),F.bgScale=Math.max(60,Math.min(120,parseInt(F.bgScale||100,10)||100)),F}catch(w){return u}})(),K=["\uB287\uCE90\uC2AC","\uB274\uCEA3\uC2AC","\uCE84\uBAAC\uC2A4\uD0C0\uC988","\uCF00\uC774\uB300","\uC5E0\uBE44\uB300","\uC640\uD50C\uB300","\uC218\uC220\uB300","\uD751\uCE74\uB370\uBBF8"],me=E.includes("\uBAAC\uC2A4\uD0C0")||q.includes("MONSTAR"),re=q==="HM"||q==="DM"||q==="SSG"||q==="JSA"||q==="BGM"||K.includes(E)||me,ne="44% 50%",ae=(()=>{const u=q==="JSA"||q==="BGM"||K.includes(E)||me;return q==="JSA"||E==="\uD751\uCE74\uB370\uBBF8"?"min(72%,620px) auto":E==="\uC218\uC220\uB300"||E==="\uC5E0\uBE44\uB300"||E==="\uCF00\uC774\uB300"?"min(84%,740px) auto":u?"min(78%,680px) auto":"min(86%,760px) auto"})(),ve=(()=>{const u=(N.bgScale||100)/100;if(u===1)return ae;const w=String(ae||"").match(/min\(\s*(\d+)\s*%\s*,\s*(\d+)\s*px\s*\)/i);if(!w)return ae;const C=Math.max(10,Math.min(100,Math.round(parseInt(w[1],10)*u))),V=Math.max(80,Math.min(1200,Math.round(parseInt(w[2],10)*u)));return`min(${C}%,${V}px) auto`})(),oe=re?String(Math.min(.48,parseFloat(j)||.48).toFixed(2)):j,Q=_b2GetUnivProfileViewMode(),he=o.bgImg?c?re?`<img src="${o.bgImg}" crossorigin="anonymous" style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:${ve.replace(" auto","")};max-width:760px;max-height:78%;height:auto;object-fit:contain;object-position:${ne};opacity:${oe};pointer-events:none;z-index:0" onerror="this.style.display='none'">`:`<img src="${o.bgImg}" crossorigin="anonymous" class="b2-fit-auto" data-fit-kind="bg" data-fit-mode="${_}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:${_==="auto"?"cover":_};object-position:${H};opacity:${oe};pointer-events:none;z-index:0" onload="_b2ApplyBgAutoSizing(this)">`:re?`<div class="b2-bg-layer" data-bg-src="${String(o.bgImg).replace(/"/g,"&quot;")}" data-bg-pos="${ne}" style="position:absolute;inset:0;opacity:${oe};pointer-events:none;z-index:0;background-position:${ne};background-size:${ve};background-repeat:no-repeat"></div>`:`<div class="b2-bg-layer" data-bg-src="${String(o.bgImg).replace(/"/g,"&quot;")}" data-bg-pos="${String(H).replace(/"/g,"&quot;")}" data-bg-size-mode="${_}" style="position:absolute;inset:0;opacity:${oe};pointer-events:none;z-index:0;background-position:${H};background-size:${_==="auto"?"cover":_};background-repeat:no-repeat"></div>`:"";let se="",ee=!1;P.forEach(u=>{const w=A[u],C=_b2RenderUnivGroupCards(w,t,!0,Q,Q==="table"&&ee);Q==="table"&&w.length&&(ee=!0),se+=I(u,!0,C)}),l.forEach(u=>{const w=M[u];w.sort((V,G)=>(V.name||"").localeCompare(G.name||"","ko",{sensitivity:"base"}));const C=_b2RenderUnivGroupCards(w,t,!1,Q,Q==="table"&&ee);Q==="table"&&w.length&&(ee=!0),se+=I(u,!1,C)});const ze=$?`<div style="margin-top:10px;background:${m};padding:12px;box-sizing:border-box;overflow:hidden;border-radius:18px;border:1px solid ${d};box-shadow:0 14px 26px rgba(15,23,42,.06)">
    ...
    ${x("\uC9C1\uCC45\uC790",b.length)}
    ${x("\uC77C\uBC18 \uC2A4\uD2B8\uB9AC\uBA38",a.length)}
    ${g.length?`<div style="border-top:1px solid var(--border2);margin:6px 0"></div>${g.map(s=>x(s,f[s],getTierBtnColor(s))).join("")}`:""}`,n._forEl=e,document.body.appendChild(n);const d=e.getBoundingClientRect();n.style.top=d.bottom+6+"px",n.style.left=d.left+"px",requestAnimationFrame(()=>{d.left+n.offsetWidth>window.innerWidth-8&&(n.style.left=d.right-n.offsetWidth+"px"),d.bottom+n.offsetHeight+6>window.innerHeight&&(n.style.top=d.top-n.offsetHeight-6+"px")}),setTimeout(()=>{function s(k){!n.contains(k.target)&&k.target!==e&&T()}function m(){T()}function T(){n.remove(),document.removeEventListener("click",s),window.removeEventListener("scroll",m,!0)}document.addEventListener("click",s),window.addEventListener("scroll",m,{capture:!0,once:!0})},0)}async function saveB2Img(){const e=_b2VisUnivs().filter(n=>n.name!=="\uBB34\uC18C\uC18D"),t=_b2SaveUniv==="\uC804\uCCB4"?e:e.filter(n=>n.name===_b2SaveUniv);if(!t.length){alert("\uC800\uC7A5\uD560 \uB300\uD559\uC774 \uC5C6\uC2B5\uB2C8\uB2E4.");return}const r=document.querySelector('[onclick="saveB2Img()"]');r&&(r.disabled=!0,r.textContent="\u23F3...");const c=720,o=14,b=24,a=document.createElement("div");a.style.cssText=`position:fixed;left:-9999px;top:0;padding:${b}px;background:#f0f2f5;box-sizing:border-box;width:${c+b*2}px`,a.innerHTML=`<style>.b2-bottom-img{max-width:160px;max-height:130px;object-fit:contain;}</style>
    ...
    </div>`,document.body.appendChild(a),a.querySelectorAll(".no-export,.no-export-movebtns").forEach(n=>n.remove()),await new Promise(n=>setTimeout(n,100)),injectUnivIcons(a);const f=a.scrollHeight+32,g=a.scrollWidth,x=(_b2SaveUniv==="\uC804\uCCB4"?"\uB300\uD559\uBCC4\uD604\uD669\uD310_\uC804\uCCB4":`\uB300\uD559\uBCC4\uD604\uD669\uD310_${_b2SaveUniv}`)+"_"+new Date().toISOString().slice(0,10)+".png";try{if(typeof _captureAndSave!="function")throw new Error("\uC774\uBBF8\uC9C0 \uC800\uC7A5 \uAE30\uB2A5\uC744 \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.");await _captureAndSave(a,g,f,x)}catch(n){console.error("[\uD604\uD669\uD310 \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2E4\uD328]",n),alert(`\u274C \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2E4\uD328

`+(n.message||"\uC54C \uC218 \uC5C6\uB294 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4."))}finally{document.body.removeChild(a),r&&(r.disabled=!1,r.textContent="\u{1F4F7} \uC774\uBBF8\uC9C0\uC800\uC7A5")}}function _b2PastelBg(e,t){const{r,g:c,b:o}=typeof _hexToRgbObj=="function"?_hexToRgbObj(e):{r:100,g:116,b:139},b=typeof t=="number"?t:.1,a=f=>Math.round(255*(1-b)+f*b);return`rgb(${a(r)},${a(c)},${a(o)})`}function _b2GetUnivProfileViewMode(){try{const e=String(localStorage.getItem("su_b2_univ_profile_view")||"").trim();return e==="card"?"poster":e==="compact"||e==="media"||e==="board"||e==="split"?"rank":["default","poster","rank","glass","table"].includes(e)?e:"default"}catch(e){return"default"}}function _b2SetUnivProfileViewMode(e){const t=["default","poster","rank","glass","table"].includes(String(e||""))?String(e):"default";try{localStorage.setItem("su_b2_univ_profile_view",t)}catch(r){}typeof render=="function"&&render()}function _b2UnivRankRow(e,t,r,c){const o=(e.name||"").replace(/'/g,"\\'"),b=e.photo?toHttpsUrl(e.photo):"",a=e.race&&e.race!=="N"?e.race:"?",f={T:"#2563eb",P:"#d97706",Z:"#7c3aed"}[e.race]||"#475569",g=r&&(e.tier||e.role)||"",x=e.tier&&typeof getTierBtnColor=="function"?getTierBtnColor(e.tier):t,n=e.tier&&typeof getTierBtnTextColor=="function"&&getTierBtnTextColor(e.tier)||"#fff",d=Number(e.win||0),s=Number(e.loss||0),m=d+s,T=m?Math.round(d/m*100):null,k=T==null?"#94a3b8":T>=50?"#16a34a":"#dc2626",h=m?`${d}\uC2B9 ${s}\uD328`:"\uAE30\uB85D \uC5C6\uC74C";return`
    <div style="display:flex;align-items:center;gap:12px;padding:9px 14px;border-radius:16px;border:1px solid ${t}22;background:linear-gradient(120deg,${t}14 0%,${t}05 100%);box-shadow:0 6px 16px rgba(15,23,42,.06);cursor:pointer;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease"
      ...
      <div style="min-width:0;flex:0 0 auto;width:112px">
        ...
        <div style="display:flex;align-items:center;gap:5px;margin-top:3px;flex-wrap:wrap">
          ...
          ${g?`<span style="display:inline-flex;padding:1px 6px;border-radius:999px;background:${x};color:${n};font-size:9px;font-weight:900">${g}</span>`:""}
        </div>
      </div>
    ...
    </div>`}function _b2UnivGlassCard(e,t,r){const c=(e.name||"").replace(/'/g,"\\'"),o=e.photo?toHttpsUrl(e.photo):"",b=e.race&&e.race!=="N"?e.race:"?",a={T:"#2563eb",P:"#d97706",Z:"#7c3aed"}[e.race]||"#475569",f=r&&(e.tier||e.role)||"",g=e.tier&&typeof getTierBtnColor=="function"?getTierBtnColor(e.tier):t,x=Number(e.win||0),n=Number(e.loss||0),d=x+n,s=d?Math.round(x/d*100):null,m=s==null?"#94a3b8":s>=50?"#16a34a":"#dc2626";return`
      ...
      <div style="padding:9px 11px 10px;background:rgba(255,255,255,.7);backdrop-filter:blur(10px) saturate(1.3);-webkit-backdrop-filter:blur(10px) saturate(1.3);border-top:1px solid ${t}20">
        ...
        <div style="margin-top:6px;display:flex;align-items:center;gap:6px">
          <div style="flex:1;height:5px;border-radius:999px;background:${t}18;overflow:hidden">
            <div style="height:100%;width:${s==null?0:s}%;border-radius:999px;background:${m}"></div>
          </div>
          <span style="font-size:10px;font-weight:900;color:${m};flex-shrink:0">${s==null?"-":s+"%"}</span>
        </div>
      </div>
    </div>`}function _b2UnivFrameCard(e,t,r){const c=(e.name||"").replace(/'/g,"\\'"),o=e.photo?toHttpsUrl(e.photo):"",b=e.race&&e.race!=="N"?e.race:"?",a={T:"#2563eb",P:"#d97706",Z:"#7c3aed"}[e.race]||"#475569",f=r&&(e.tier||e.role)||"",g=e.tier&&typeof getTierBtnColor=="function"?getTierBtnColor(e.tier):t,x=e.tier&&typeof getTierBtnTextColor=="function"&&getTierBtnTextColor(e.tier)||"#fff",n=Number(e.win||0),d=Number(e.loss||0),s=n+d,m=s?Math.round(n/s*100):null;return`
    ...
    <td><div class="b2-lc4-wrcell">
      <div class="b2-lc4-bartrack"><div class="b2-lc4-barfill" style="width:${x==null?0:x}%;background:${n}"></div></div>
      <span class="b2-lc4-wr" style="color:${n}">${x==null?"-":x+"%"}</span>
    </div></td>
  </tr>`}function _b2LineupTable(e,t,r,c,o){if(!e.length)return"";const b=r?`<div class="b2-lc4-head"><img src="${toHttpsUrl(r)}" alt="" onerror="this.style.display='none'"><span>${c||""}</span></div>`:"";return`<div class="b2-lc4-wrap" style="--lc-col:${t}">
    ${b}
    <table class="b2-lc4">
      ${o?"":"<thead><tr><th>\uC774\uB984</th><th>\uC5ED\uD560</th><th>\uD2F0\uC5B4</th><th>\uC885\uC871</th><th>\uC804\uC801</th><th>\uC2B9\uB960</th></tr></thead>"}
      <tbody>${e.map(a=>_b2LineupTableRow(a,t)).join("")}</tbody>
    </table>
  </div>`}function _b2LineupCard(e,t,r,c){const o=(e.name||"").replace(/'/g,"\\'"),b=e.race&&e.race!=="N"?e.race:"?",a=e.photo?toHttpsUrl(e.photo):"",f={T:"#2563eb",P:"#d97706",Z:"#7c3aed"}[e.race]||"#475569",g=r?e.role||"":e.tier||"",x=!r&&e.tier&&typeof getTierBtnColor=="function"?getTierBtnColor(e.tier):t,n=!r&&e.tier&&typeof getTierBtnTextColor=="function"&&getTierBtnTextColor(e.tier)||"#fff",d=a?`<img src="${a}" crossorigin="anonymous" loading="lazy" decoding="async" aria-hidden="true" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:top center;transform:scale(1.22);filter:blur(16px) saturate(1.15) brightness(.8);opacity:.85" onerror="this.style.display='none'">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;z-index:2;padding:12px 14px 13px">
      ${g?`<div style="margin-bottom:4px"><span style="background:${x};color:${n};font-weight:900;font-size:13px;padding:2px 9px;border-radius:999px;white-space:nowrap;line-height:1.6;letter-spacing:-.01em">${g}</span></div>`:""}
    ...
    </div>`}function _b2LineupView(){const e=_b2VisUnivs().filter(r=>r.name!=="\uBB34\uC18C\uC18D");if(!e.length)return'<div style="text-align:center;color:var(--text3);padding:40px">\uB4F1\uB85D\uB41C \uB300\uD559\uC774 \uC5C6\uC2B5\uB2C8\uB2E4</div>';(!_b2LineupUniv||!e.some(r=>r.name===_b2LineupUniv))&&(_b2LineupUniv=e[0].name);const t=gc(_b2LineupUniv);return`<div style="max-width:1360px;margin:0 auto">${_b2LineupPoster(_b2LineupUniv,t,!1)}</div>`}async function _b2CaptureBoardHtml({btnSelector:e,cardWidth:t,pad:r,innerHtml:c,heightPad:o,filename:b,errorLabel:a}){const f=document.querySelector(e);f&&(f.disabled=!0,f.textContent="\u23F3...");const g=document.createElement("div");g.style.cssText=`position:fixed;left:-9999px;top:0;padding:${r}px;background:#f0f2f5;box-sizing:border-box;width:${t+r*2}px`,g.innerHTML=c,document.body.appendChild(g),g.querySelectorAll(".no-export,.no-export-movebtns").forEach(d=>d.remove()),await new Promise(d=>setTimeout(d,100)),injectUnivIcons(g);const x=g.scrollHeight+o,n=g.scrollWidth;try{if(typeof _captureAndSave!="function")throw new Error("\uC774\uBBF8\uC9C0 \uC800\uC7A5 \uAE30\uB2A5\uC744 \uBD88\uB7EC\uC624\uC9C0 \uBABB\uD588\uC2B5\uB2C8\uB2E4.");await _captureAndSave(g,n,x,b)}catch(d){console.error(`[${a} \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2E4\uD328]`,d),alert(`\u274C \uC774\uBBF8\uC9C0 \uC800\uC7A5 \uC2E4\uD328

`+(d.message||"\uC54C \uC218 \uC5C6\uB294 \uC624\uB958\uAC00 \uBC1C\uC0DD\uD588\uC2B5\uB2C8\uB2E4."))}finally{document.body.removeChild(g),f&&(f.disabled=!1,f.textContent="\u{1F4F7} \uC774\uBBF8\uC9C0\uC800\uC7A5")}}async function saveB2LineupImg(){const e=_b2VisUnivs().filter(r=>r.name!=="\uBB34\uC18C\uC18D");if((!_b2LineupUniv||!e.some(r=>r.name===_b2LineupUniv))&&(_b2LineupUniv=e[0]?e[0].name:""),!_b2LineupUniv){alert("\uC800\uC7A5\uD560 \uB300\uD559\uC774 \uC5C6\uC2B5\uB2C8\uB2E4.");return}const t=gc(_b2LineupUniv);await _b2CaptureBoardHtml({btnSelector:'[onclick="saveB2LineupImg()"]',cardWidth:1360,pad:0,innerHtml:_b2LineupPoster(_b2LineupUniv,t,!0),heightPad:8,filename:`\uB300\uD559\uB77C\uC778\uC5C5_${_b2LineupUniv}_`+new Date().toISOString().slice(0,10)+".png",errorLabel:"\uB77C\uC778\uC5C5"})}async function saveB2FreeImg(){await _b2CaptureBoardHtml({btnSelector:'[onclick="saveB2FreeImg()"]',cardWidth:720,pad:24,innerHtml:`<style>.b2-bottom-img{max-width:160px;max-height:130px;object-fit:contain;}</style>${_b2FreeView()}`,heightPad:32,filename:"\uBB34\uC18C\uC18D\uD604\uD669\uD310_"+new Date().toISOString().slice(0,10)+".png",errorLabel:"\uBB34\uC18C\uC18D \uD604\uD669\uD310"})}function _b2ContrastColor(e){try{const t=String(e||"").replace("#","").trim(),r=parseInt(t.slice(0,2),16),c=parseInt(t.slice(2,4),16),o=parseInt(t.slice(4,6),16);if([r,c,o].some(x=>Number.isNaN(x)))return"#ffffff";const b=x=>(x/=255,x<=.03928?x/12.92:Math.pow((x+.055)/1.055,2.4)),a=.2126*b(r)+.7152*b(c)+.0722*b(o),f=(1+.05)/(a+.05),g=(a+.05)/(.02+.05);return f>=g?"#ffffff":"#0f172a"}catch(t){return"#ffffff"}}localStorage.removeItem("su_b2SelectedPlayer"),(function(){const e=players.filter(c=>c&&!c.hidden&&!c.retired&&!c.hideFromBoard),t=e.filter(c=>c.photo||window.playerPhotos&&window.playerPhotos[c.name]),r=t.length?t:e;r.length&&(_b2SelectedPlayer=r[Math.floor(Math.random()*r.length)])})();
