...
var _b2LiveUnivFilter="\uC804\uCCB4",_b2LiveTierFilter="\uC804\uCCB4",_b2LiveSearch="",_b2LiveSortMode=(()=>{try{return localStorage.getItem("su_b2_live_sort")||"tier"}catch(e){return"tier"}})(),_b2LiveCardSize=(()=>{try{const s=localStorage.getItem("su_b2_live_card_size");return["s","m","l"].includes(s)?s:"m"}catch(e){return"m"}})(),_b2LiveObserver=null,_b2LiveStatusCache={},_b2LivePollTimer=null,_b2LivePollInFlight=!1,_b2LivePollIntervalMs=45e3,_b2LiveHoverOpenTimer=null,_b2LiveHoverCloseTimer=null,_b2LiveHoverOpenId="",_b2LiveHoverOpenName="";function _b2LiveSoopId(input){const s=String(input||"").trim();if(!s)return"";try{const u=new URL(s,window.location.href),host=(u.hostname||"").toLowerCase();if(host.includes("sooplive.co.kr")||host.includes("sooplive.com")||host.includes("afreecatv.com")){const m=(u.pathname||"").match(/^\/([^\/]+)/);return m?m[1]:""}}catch(e){}return""}function _b2LiveEmbedUrl(id){return id?`https://play.sooplive.co.kr/${id}/embed?mute=y&showChat=false`:""}function _b2LiveSetSort(mode){_b2LiveSortMode=["tier","name","univ"].includes(mode)?mode:"tier";try{localStorage.setItem("su_b2_live_sort",_b2LiveSortMode)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveSetCardSize(size){_b2LiveCardSize=["s","m","l"].includes(size)?size:"m";try{localStorage.setItem("su_b2_live_card_size",_b2LiveCardSize)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveInitObservers(container){try{if(!container)return;if(_b2LiveObserver){try{_b2LiveObserver.disconnect()}catch(e){}_b2LiveObserver=null}const boxes=container.querySelectorAll(".b2-live-frame-box[data-src]");if(!boxes.length)return;if(!window.IntersectionObserver){boxes.forEach(b=>{const f=b.querySelector("iframe");f&&!f.src&&(f.src=b.dataset.src)});return}_b2LiveObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(!entry.isIntersecting)return;const box=entry.target,f=box.querySelector("iframe");f&&!f.src&&(f.src=box.dataset.src),_b2LiveObserver.unobserve(box)})},{root:null,rootMargin:"200px 0px",threshold:.01}),boxes.forEach(b=>_b2LiveObserver.observe(b))}catch(e){}}function _b2LiveEnlarge(id,name){try{if(!id)return;let ov=document.getElementById("b2LiveEnlargeOverlay");ov||(ov=document.createElement("div"),ov.id="b2LiveEnlargeOverlay",ov.className="su-modal-overlay",document.body.appendChild(ov)),ov.style.display="flex",ov.innerHTML=`
  ...
  `;if(!soopPlayers.length)return`
      ...
      <div style="padding:60px 20px;text-align:center;color:var(--gray-l)">
        ...
        <div style="font-weight:700">\uD574\uB2F9 \uC870\uAC74\uC5D0 \uB9DE\uB294 \uC2A4\uD2B8\uB9AC\uBA38\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4</div>
         ...
         <span style="display:none;width:100%;height:100%;align-items:center;justify-content:center;background:${univColor}18;color:${univColor};font-size:18px;font-weight:1000">${safeNameHtml.slice(0,1)||"?"}</span>`:`<span style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:${univColor}18;color:${univColor};font-size:18px;font-weight:1000">${safeNameHtml.slice(0,1)||"?"}</span>`;let groupHeader="";if(showTierGroups){const tk=p.tier||"__none__";if(tk!==_lastTierKey){const isFirst=_lastTierKey===void 0;_lastTierKey=tk;const label=p.tier?typeof getTierLabel=="function"?getTierLabel(p.tier):p.tier:"\uBBF8\uC9C0\uC815";groupHeader=`
          ...
          </div>`}}const enlargeBtn=`<button type="button" onclick="_b2LiveEnlarge('${p._soopId}','${safeName}')" title="\uD655\uB300\uBCF4\uAE30"
            style="width:${sizeCfg.stacked?22:26}px;height:${sizeCfg.stacked?22:26}px;border-radius:7px;border:1px solid var(--border2);background:var(--white);color:var(--text2);font-size:${sizeCfg.stacked?11:12}px;cursor:pointer;line-height:1;padding:0;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center">\u26F6</button>`,linkBtn=`<a href="https://ch.sooplive.co.kr/${p._soopId}" target="_blank" rel="noopener"
            style="width:${sizeCfg.stacked?22:26}px;height:${sizeCfg.stacked?22:26}px;border-radius:7px;border:1px solid var(--border2);background:var(--white);font-size:${sizeCfg.stacked?11:12}px;text-decoration:none;flex-shrink:0;display:inline-flex;align-items:center;justify-content:center" title="SOOP \uCC44\uB110\uB85C \uC774\uB3D9">\u{1F517}</a>`,avatarBlock=`
      <div style="position:relative;flex-shrink:0" onmouseenter="_b2LiveHoverEnter(this,'${p._soopId}','${safeName}')" onmouseleave="_b2LiveHoverLeave(event)">
        <button type="button" onclick="openPlayerModal&&openPlayerModal('${safeName}')" title="${badgeInitDisplay==="inline-flex"?"\uC120\uC218 \uC0C1\uC138 \xB7 \uB9C8\uC6B0\uC2A4 \uC62C\uB9AC\uBA74 \uB77C\uC774\uBE0C \uBBF8\uB9AC\uBCF4\uAE30":"\uC120\uC218 \uC0C1\uC138"}"
