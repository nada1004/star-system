...
var _b2LiveUnivFilter="\uC804\uCCB4",_b2LiveTierFilter="\uC804\uCCB4",_b2LiveSearch="",_b2LiveSortMode=(()=>{try{return localStorage.getItem("su_b2_live_sort")||"tier"}catch(e){return"tier"}})(),_b2LiveCardSize=(()=>{try{const s=localStorage.getItem("su_b2_live_card_size");return["s","m","l"].includes(s)?s:"m"}catch(e){return"m"}})(),_b2LiveObserver=null,_b2LiveHideOffline=(()=>{try{return localStorage.getItem("su_b2_live_hide_offline")==="1"}catch(e){return!1}})(),_b2LiveStatusCache={},_b2LivePollTimer=null,_b2LivePollInFlight=!1,_b2LivePollIntervalMs=45e3,_b2LiveHoverOpenTimer=null,_b2LiveHoverCloseTimer=null,_b2LiveHoverOpenId="",_b2LiveHoverOpenName="";function _b2LiveSoopId(input){const s=String(input||"").trim();if(!s)return"";try{const u=new URL(s,window.location.href),host=(u.hostname||"").toLowerCase();if(host.includes("sooplive.co.kr")||host.includes("sooplive.com")||host.includes("afreecatv.com")){const m=(u.pathname||"").match(/^\/([^\/]+)/);return m?m[1]:""}}catch(e){}return""}function _b2LiveEmbedUrl(id){return id?`https://play.sooplive.co.kr/${id}/embed?mute=y&showChat=false`:""}function _b2LiveSetSort(mode){_b2LiveSortMode=["tier","name","univ"].includes(mode)?mode:"tier";try{localStorage.setItem("su_b2_live_sort",_b2LiveSortMode)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveSetCardSize(size){_b2LiveCardSize=["s","m","l"].includes(size)?size:"m";try{localStorage.setItem("su_b2_live_card_size",_b2LiveCardSize)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveInitObservers(container){try{if(!container)return;if(_b2LiveObserver){try{_b2LiveObserver.disconnect()}catch(e){}_b2LiveObserver=null}const boxes=container.querySelectorAll(".b2-live-frame-box[data-src]");if(!boxes.length)return;if(!window.IntersectionObserver){boxes.forEach(b=>{const f=b.querySelector("iframe");f&&!f.src&&(f.src=b.dataset.src)});return}_b2LiveObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(!entry.isIntersecting)return;const box=entry.target,f=box.querySelector("iframe");f&&!f.src&&(f.src=box.dataset.src),_b2LiveObserver.unobserve(box)})},{root:null,rootMargin:"200px 0px",threshold:.01}),boxes.forEach(b=>_b2LiveObserver.observe(b))}catch(e){}}function _b2LiveEnlarge(id,name){try{if(!id)return;let ov=document.getElementById("b2LiveEnlargeOverlay");ov||(ov=document.createElement("div"),ov.id="b2LiveEnlargeOverlay",ov.className="su-modal-overlay",document.body.appendChild(ov)),ov.style.display="flex",ov.innerHTML=`
    ...
    `,ov.onclick=e=>{e.target===ov&&(ov.style.display="none",ov.innerHTML="")}}catch(e){}}function _b2LiveCanHoverPreview(){try{return!!(window.matchMedia&&window.matchMedia("(hover: hover) and (pointer: fine)").matches)}catch(e){return!1}}function _b2LiveEnsureHoverOverlay(){let ov=document.getElementById("b2LiveHoverOverlay");return ov||(ov=document.createElement("div"),ov.id="b2LiveHoverOverlay",ov.className="su-modal-overlay",ov.style.display="none",ov.addEventListener("mousedown",e=>{try{e.target===ov&&_b2LiveHideHoverPreview(!0)}catch(_){}}),document.body.appendChild(ov),ov)}function _b2LiveShowHoverPreview(anchorEl,id,name){try{if(!_b2LiveCanHoverPreview()||!anchorEl||!id)return;const st=_b2LiveStatusCache[id];if(!(st&&st.live))return;const ov=_b2LiveEnsureHoverOverlay(),safeName=String(name||"").replace(/'/g,"\\'");_b2LiveHoverOpenId=String(id||""),_b2LiveHoverOpenName=String(name||""),ov.style.display="flex",ov.innerHTML=`
      <div class="su-modal" style="width:min(1100px, calc(100vw - 28px));height:min(640px, calc(100vh - 28px));overflow:hidden">
        <div class="su-modal-hd" style="display:flex;align-items:center;justify-content:space-between">
          ...
          <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
            <a href="https://ch.sooplive.co.kr/${id}" target="_blank" rel="noopener" class="btn btn-w btn-sm" style="text-decoration:none">\uC2E4\uC2DC\uAC04 Catch \uBCF4\uAE30</a>
            <button type="button" class="btn btn-r btn-sm" onclick="_b2LiveHideHoverPreview(true)">\uB2EB\uAE30</button>
          </div>
        </div>
        <div class="su-modal-bd" style="padding:0;overflow:hidden;flex:1;min-height:0">
          <iframe src="${_b2LiveEmbedUrl(id)}" allow="autoplay; fullscreen; picture-in-picture" referrerpolicy="no-referrer"
            style="width:100%;height:100%;border:0;background:#000"></iframe>
        </div>
      </div>
    `;try{const modal=ov.querySelector(".su-modal");modal&&!modal.dataset.boundHover&&(modal.dataset.boundHover="1",modal.addEventListener("mouseenter",()=>{try{clearTimeout(_b2LiveHoverCloseTimer)}catch(e){}}),modal.addEventListener("mouseleave",()=>{_b2LiveHoverLeave()}))}catch(e){}}catch(e){}}function _b2LiveHideHoverPreview(immediate){try{clearTimeout(_b2LiveHoverOpenTimer),clearTimeout(_b2LiveHoverCloseTimer);const ov=document.getElementById("b2LiveHoverOverlay");if(!ov)return;const close=()=>{ov.style.display="none",ov.innerHTML="",_b2LiveHoverOpenId="",_b2LiveHoverOpenName=""};immediate?close():_b2LiveHoverCloseTimer=setTimeout(close,180)}catch(e){}}function _b2LiveHoverEnter(anchorEl,id,name){try{clearTimeout(_b2LiveHoverCloseTimer),clearTimeout(_b2LiveHoverOpenTimer),_b2LiveHoverOpenTimer=setTimeout(()=>_b2LiveShowHoverPreview(anchorEl,id,name),140)}catch(e){}}function _b2LiveHoverLeave(){_b2LiveHideHoverPreview(!1)}function _b2LiveToggleHideOffline(){_b2LiveHideOffline=!_b2LiveHideOffline;try{localStorage.setItem("su_b2_live_hide_offline",_b2LiveHideOffline?"1":"0")}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}async function _b2LiveFetchStatus(ids){if(!ids||!ids.length)return{};try{const res=await fetch("/api/soop-live-status",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ids})});if(!res.ok)return{};const data=await res.json().catch(()=>null);return data&&data.results||{}}catch(e){return{}}}function _b2LiveApplyStatusToDom(container){if(!container)return;const cards=container.querySelectorAll(".b2-live-card[data-soop-id]");let liveCount=0;cards.forEach(card=>{const id=card.getAttribute("data-soop-id"),st=_b2LiveStatusCache[id],badge=card.querySelector(".b2-live-badge"),avatarBadge=card.querySelector(".b2-live-preview-badge");st&&st.live?(liveCount++,badge&&(badge.style.display="inline-flex"),avatarBadge&&(avatarBadge.style.display="inline-flex"),card.style.display=""):st?(badge&&(badge.style.display="none"),avatarBadge&&(avatarBadge.style.display="none"),card.style.display=_b2LiveHideOffline?"none":""):(badge&&(badge.style.display="none"),avatarBadge&&(avatarBadge.style.display="none"))});const counter=document.getElementById("b2-live-count-badge");counter&&(counter.textContent=`\u{1F534} \uB77C\uC774\uBE0C ${liveCount}\uBA85`)}async function _b2LivePoll(){if(_b2LivePollInFlight)return;const container=document.getElementById("b2-content"),cards=container?container.querySelectorAll(".b2-live-card[data-soop-id]"):null;if(!container||!cards||!cards.length){_b2LiveStopPoll();return}const ids=Array.from(cards).map(c=>c.getAttribute("data-soop-id")).filter(Boolean);_b2LivePollInFlight=!0;try{const results=await _b2LiveFetchStatus(ids),now=Date.now();Object.keys(results).forEach(id=>{_b2LiveStatusCache[id]=Object.assign({ts:now},results[id])});const stillLive=document.getElementById("b2-content");stillLive&&stillLive.querySelector(".b2-live-card[data-soop-id]")&&_b2LiveApplyStatusToDom(stillLive)}finally{_b2LivePollInFlight=!1}}function _b2LiveStartPoll(){_b2LiveStopPoll(),_b2LivePoll(),_b2LivePollTimer=setInterval(_b2LivePoll,_b2LivePollIntervalMs)}function _b2LiveStopPoll(){_b2LivePollTimer&&(clearInterval(_b2LivePollTimer),_b2LivePollTimer=null)}function _b2LiveView(){const dissolvedUnivs=typeof univCfg!="undefined"?new Set((univCfg.filter(u=>u.dissolved)||[]).map(u=>u.name)):new Set,soopPlayers=(typeof players!="undefined"?players:[]).filter(p=>{if(p.hidden||p.retired||p.hideFromBoard)return!1;const _u=String((p==null?void 0:p.univ)||"").trim();return _u==="YB"||_u==="\uBB34\uC18C\uC18D"||!_u||dissolvedUnivs.has(p.univ)?!1:!!_b2LiveSoopId(p.channelUrl)}).map(p=>Object.assign({_soopId:_b2LiveSoopId(p.channelUrl)},p)),univList=[...new Set(soopPlayers.map(p=>String((p==null?void 0:p.univ)||"").trim()).filter(Boolean))];typeof univCfg!="undefined"?univList.sort((a,b)=>{const idxA=univCfg.findIndex(u=>u.name===a),idxB=univCfg.findIndex(u=>u.name===b);return(idxA>=0?idxA:999)-(idxB>=0?idxB:999)}):univList.sort();const univFiltered=_b2LiveUnivFilter==="\uC804\uCCB4"?soopPlayers:soopPlayers.filter(p=>String((p==null?void 0:p.univ)||"").trim()===_b2LiveUnivFilter),tierFiltered0=_b2LiveTierFilter==="\uC804\uCCB4"?univFiltered:univFiltered.filter(p=>p.tier===_b2LiveTierFilter),q=String(_b2LiveSearch||"").trim().toLowerCase(),searched=q?tierFiltered0.filter(p=>String(p.name||"").toLowerCase().includes(q)):tierFiltered0,tierOrder=typeof TIERS!="undefined"?TIERS:[],univOrder=typeof univCfg!="undefined"?univCfg.map(u=>u.name):[],tierFiltered=searched.slice().sort((a,b)=>{if(_b2LiveSortMode==="name")return String(a.name||"").localeCompare(String(b.name||""),"ko");if(_b2LiveSortMode==="univ"){const ia=univOrder.indexOf(a.univ),ib=univOrder.indexOf(b.univ),ra=ia>=0?ia:999,rb=ib>=0?ib:999;return ra!==rb?ra-rb:String(a.name||"").localeCompare(String(b.name||""),"ko")}const idxA=tierOrder.indexOf(a.tier),idxB=tierOrder.indexOf(b.tier),rankA=idxA>=0?idxA:999,rankB=idxB>=0?idxB:999;return rankA!==rankB?rankA-rankB:String(a.name||"").localeCompare(String(b.name||""),"ko")}),liveFiltered=_b2LiveHideOffline?tierFiltered.filter(p=>{const st=_b2LiveStatusCache[p._soopId];return!st||st.live}):tierFiltered,knownLiveCount=tierFiltered.reduce((n,p)=>n+(_b2LiveStatusCache[p._soopId]&&_b2LiveStatusCache[p._soopId].live?1:0),0),unknownCount=tierFiltered.reduce((n,p)=>n+(_b2LiveStatusCache[p._soopId]?0:1),0),cardMinPx={s:170,m:240,l:320}[_b2LiveCardSize]||240,sizeBtn=(v,label)=>`<button type="button" class="b2-toolbar-btn" onclick="_b2LiveSetCardSize('${v}')"
    style="padding:4px 10px;border-radius:8px;border:1px solid ${_b2LiveCardSize===v?"#2563eb":"var(--border2)"};background:${_b2LiveCardSize===v?"linear-gradient(135deg,#eff6ff,#dbeafe)":"var(--white)"};color:${_b2LiveCardSize===v?"#1d4ed8":"var(--text2)"};font-size:var(--fs-sm);font-weight:${_b2LiveCardSize===v?900:700};cursor:pointer;margin-bottom:0">${label}</button>`,hideOfflineBtn=`<button type="button" class="b2-toolbar-btn" onclick="_b2LiveToggleHideOffline()" title="\uC624\uD504\uB77C\uC778 \uCE74\uB4DC \uC228\uAE30\uAE30"
    ...
    ${_b2LiveHideOffline?"\u{1F648}":"\u{1F441}\uFE0F"} \uC624\uD504\uB77C\uC778 \uC228\uAE30\uAE30
  </button>`,filterBar=`
    <style>
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px">
      ...
      ${hideOfflineBtn}
      <button type="button" class="b2-toolbar-btn" onclick="_b2LivePoll()" title="\uB77C\uC774\uBE0C \uC0C1\uD0DC \uC0C8\uB85C\uACE0\uCE68"
    style="padding:4px 10px;border-radius:8px;border:1px solid var(--border2);background:var(--white);color:var(--text2);font-size:var(--fs-sm);font-weight:700;cursor:pointer">\u{1F504}</button>
      <div style="margin-left:auto;display:flex;align-items:center;gap:10px;font-size:var(--fs-sm);color:var(--gray-l);font-weight:700">
        <span id="b2-live-count-badge">\u{1F534} \uB77C\uC774\uBE0C ${knownLiveCount}\uBA85</span>
        ${unknownCount>0?`<span title="\uC544\uC9C1 \uBC29\uC1A1\uC0C1\uD0DC \uD655\uC778\uC774 \uC548 \uB41C \uC2A4\uD2B8\uB9AC\uBA38 \u2014 \uC624\uD504\uB77C\uC778 \uC228\uAE30\uAE30 \uB300\uC0C1\uC5D0\uC11C \uC81C\uC678\uB418\uC5B4 \uACC4\uC18D \uD45C\uC2DC\uB429\uB2C8\uB2E4">\u23F3 \uBBF8\uD655\uC778 ${unknownCount}\uBA85</span>`:""}
        <span>\u{1F4FA} ${liveFiltered.length}\uBA85 \uD45C\uC2DC\uC911 (SOOP \uB4F1\uB85D ${soopPlayers.length}\uBA85)</span>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:6px;padding:8px 12px;margin-bottom:14px;background:var(--surface);border:1px dashed var(--border2);border-radius:10px;font-size:var(--fs-sm);color:var(--gray-l)">
      \u2139\uFE0F \uBC29\uC1A1\uC911 \uC5EC\uBD80\uB294 SOOP \uBE44\uACF5\uC2DD API\uB97C \uC8FC\uAE30\uC801\uC73C\uB85C \uC870\uD68C\uD574 \uD45C\uC2DC\uD558\uBBC0\uB85C \uC2E4\uC81C\uC640 \uCD5C\uB300 ${Math.round(_b2LivePollIntervalMs/1e3)}\uCD08 \uC815\uB3C4 \uCC28\uC774\uAC00 \uB0A0 \uC218 \uC788\uC2B5\uB2C8\uB2E4.
      \uC624\uD504\uB77C\uC778 \uC228\uAE30\uAE30 \uC911 \uC0C8\uB85C \uBC29\uC1A1\uC744 \uC2DC\uC791\uD55C \uC2A4\uD2B8\uB9AC\uBA38\uB294 \uD544\uD130\uB97C \uB2E4\uC2DC \uC120\uD0DD\uD558\uAC70\uB098 \u{1F504} \uC0C8\uB85C\uACE0\uCE68\uC744 \uB204\uB974\uBA74 \uB2E4\uC2DC \uB098\uD0C0\uB0A9\uB2C8\uB2E4.
      ${_b2LiveCanHoverPreview()?" \uD504\uB85C\uD544 \uC774\uBBF8\uC9C0\uC5D0 \uB9C8\uC6B0\uC2A4\uB97C \uC62C\uB9AC\uBA74 \uBC29\uC1A1 \uBBF8\uB9AC\uBCF4\uAE30\uAC00 \uC5F4\uB9BD\uB2C8\uB2E4.":""}
  ...
  `;if(!soopPlayers.length)return`
      ...
      </div>`;if(!liveFiltered.length)return filterBar+`
      <div style="padding:60px 20px;text-align:center;color:var(--gray-l)">
        <div style="font-size:40px;margin-bottom:10px">\u{1F4F4}</div>
        <div style="font-weight:700">\uD604\uC7AC \uBC29\uC1A1\uC911\uC778 \uC2A4\uD2B8\uB9AC\uBA38\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4</div>
        <div style="font-size:var(--fs-sm);margin-top:6px">\uC624\uD504\uB77C\uC778 \uC228\uAE30\uAE30\uAC00 \uCF1C\uC838 \uC788\uC2B5\uB2C8\uB2E4. \u{1F441}\uFE0F \uBC84\uD2BC\uC73C\uB85C \uB044\uBA74 \uC804\uCCB4 \uCE74\uB4DC\uB97C \uBCFC \uC218 \uC788\uC5B4\uC694.</div>
      ...
      <div class="b2-live-card" data-soop-id="${p._soopId}" style="background:var(--white);border:1.5px solid var(--border2);border-radius:14px;overflow:hidden;display:flex;flex-direction:column">
        <div style="position:relative;width:100%;aspect-ratio:16/9;background:linear-gradient(135deg,#0b1220,#111827);display:flex;align-items:center;justify-content:center">
          <div style="display:flex;flex-direction:column;align-items:center;gap:8px;color:rgba(255,255,255,.86);text-align:center;padding:14px">
            <div style="font-size:28px;font-weight:1000;letter-spacing:1px;opacity:.82">SOOP</div>
            <div style="font-size:11px;font-weight:800;opacity:.76">\uD504\uB85C\uD544 \uC774\uBBF8\uC9C0\uC5D0 \uB9C8\uC6B0\uC2A4\uB97C \uC62C\uB9AC\uBA74 \uBBF8\uB9AC\uBCF4\uAE30\uAC00 \uC5F4\uB9BD\uB2C8\uB2E4</div>
          </div>
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:8px 10px">
          <div style="position:relative;flex-shrink:0" onmouseenter="_b2LiveHoverEnter(this,'${p._soopId}','${safeName}')" onmouseleave="_b2LiveHoverLeave()">
          ...
          <a href="https://ch.sooplive.co.kr/${p._soopId}" target="_blank" rel="noopener" style="font-size:12px;text-decoration:none;flex-shrink:0" title="SOOP \uCC44\uB110\uB85C \uC774\uB3D9">\u{1F517}</a>
        </div>
      </div>
