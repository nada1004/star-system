...
var _b2LiveUnivFilter="\uC804\uCCB4",_b2LiveTierFilter="\uC804\uCCB4",_b2LiveGenderFilter=(()=>{try{const g=localStorage.getItem("su_b2_live_gender");return["\uC804\uCCB4","F","M"].includes(g)?g:"\uC804\uCCB4"}catch(e){return"\uC804\uCCB4"}})(),_b2LiveSearch="",_b2LiveSortMode=(()=>{try{return localStorage.getItem("su_b2_live_sort")||"tier"}catch(e){return"tier"}})(),_b2LiveCardSize=(()=>{try{const s=localStorage.getItem("su_b2_live_card_size");return["s","m"].includes(s)?s:"m"}catch(e){return"m"}})(),_b2LiveObserver=null,_b2LiveStatusCache={},_b2LivePollTimer=null,_b2LivePollInFlight=!1,_b2LivePollIntervalMs=45e3,_b2LiveHoverOpenTimer=null,_b2LiveHoverCloseTimer=null,_b2LiveHoverOpenId="",_b2LiveHoverOpenName="";function _b2LiveSoopId(input){const s=String(input||"").trim();if(!s)return"";try{const u=new URL(s,window.location.href),host=(u.hostname||"").toLowerCase();if(host.includes("sooplive.co.kr")||host.includes("sooplive.com")||host.includes("afreecatv.com")){const m=(u.pathname||"").match(/^\/([^\/]+)/);return m?m[1]:""}}catch(e){}return""}function _b2LiveEmbedUrl(id){return id?`https://play.sooplive.co.kr/${id}/embed?mute=y&showChat=false`:""}function _b2LiveSetSort(mode){_b2LiveSortMode=["tier","name","univ"].includes(mode)?mode:"tier";try{localStorage.setItem("su_b2_live_sort",_b2LiveSortMode)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveSetGender(g){_b2LiveGenderFilter=["\uC804\uCCB4","F","M"].includes(g)?g:"\uC804\uCCB4";try{localStorage.setItem("su_b2_live_gender",_b2LiveGenderFilter)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveSetCardSize(size){_b2LiveCardSize=["s","m"].includes(size)?size:"m";try{localStorage.setItem("su_b2_live_card_size",_b2LiveCardSize)}catch(e){}const el=document.getElementById("b2-content");el&&(el.innerHTML=_b2LiveView(),typeof injectUnivIcons=="function"&&injectUnivIcons(el))}function _b2LiveInitObservers(container){try{if(!container)return;if(_b2LiveObserver){try{_b2LiveObserver.disconnect()}catch(e){}_b2LiveObserver=null}const boxes=container.querySelectorAll(".b2-live-frame-box[data-src]");if(!boxes.length)return;if(!window.IntersectionObserver){boxes.forEach(b=>{const f=b.querySelector("iframe");f&&!f.src&&(f.src=b.dataset.src)});return}_b2LiveObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(!entry.isIntersecting)return;const box=entry.target,f=box.querySelector("iframe");f&&!f.src&&(f.src=box.dataset.src),_b2LiveObserver.unobserve(box)})},{root:null,rootMargin:"200px 0px",threshold:.01}),boxes.forEach(b=>_b2LiveObserver.observe(b))}catch(e){}}function _b2LiveEnlarge(id,name){try{if(!id)return;let ov=document.getElementById("b2LiveEnlargeOverlay");ov||(ov=document.createElement("div"),ov.id="b2LiveEnlargeOverlay",ov.className="su-modal-overlay",document.body.appendChild(ov)),ov.style.display="flex",ov.innerHTML=`
    ...
    `,ov.onclick=e=>{e.target===ov&&(ov.style.display="none",ov.innerHTML="")}}catch(e){}}function _b2LiveCanHoverPreview(){try{if(window.matchMedia&&window.matchMedia("(hover: none)").matches)return!1}catch(e){}return!0}function _b2LiveEnsureHoverOverlay(){let ov=document.getElementById("b2LiveHoverOverlay");return ov||(ov=document.createElement("div"),ov.id="b2LiveHoverOverlay",ov.className="su-modal-overlay",ov.style.display="none",ov.addEventListener("mousedown",e=>{try{e.target===ov&&_b2LiveHideHoverPreview(!0)}catch(_){}}),ov.addEventListener("mouseenter",()=>{try{clearTimeout(_b2LiveHoverCloseTimer)}catch(e){}}),ov.addEventListener("mouseleave",()=>{_b2LiveHoverLeave()}),document.body.appendChild(ov),ov)}function _b2LiveRelativeTime(startTimeStr){if(!startTimeStr)return"";try{const start=new Date(String(startTimeStr).replace(/-/g,"/")),diffMs=Date.now()-start.getTime();if(!Number.isFinite(diffMs)||diffMs<0)return"";const diffMin=Math.floor(diffMs/6e4);if(diffMin<1)return"\uBC29\uAE08 \uC804";if(diffMin<60)return`${diffMin}\uBD84 \uC804`;const diffHour=Math.floor(diffMin/60);return diffHour<24?`${diffHour}\uC2DC\uAC04 \uC804`:`${Math.floor(diffHour/24)}\uC77C \uC804`}catch(e){return""}}function _b2LiveShowHoverPreview(anchorEl,id,name){try{if(!_b2LiveCanHoverPreview()||!anchorEl||!id)return;const st=_b2LiveStatusCache[id],ov=_b2LiveEnsureHoverOverlay();if(_b2LiveHoverOpenId=String(id||""),_b2LiveHoverOpenName=String(name||""),ov.style.display="flex",!st||!st.live){ov.style.display="none";return}const safeTitle=String(st.title||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),viewerCnt=Number.isFinite(st.viewerCnt)?st.viewerCnt:st.viewerCnt||"0",relTime=_b2LiveRelativeTime(st.broadStart),thumbHtml=st.thumb?`<img src="${st.thumb}" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;display:block;background:#111" onerror="this.style.display='none'">`:'<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#94a3b8;background:#111;font-size:13px">\uBBF8\uB9AC\uBCF4\uAE30 \uC774\uBBF8\uC9C0 \uC5C6\uC74C</div>';ov.innerHTML=`
  ...
  `+`<div id="b2-live-results">${_b2LiveResultsHTML()}</div>`}function _b2LiveResultsHTML(){const soopPlayers=(typeof players!="undefined"?players:[]).filter(p=>p.hidden||p.retired||p.hideFromBoard||String((p==null?void 0:p.univ)||"").trim()==="YB"?!1:!!_b2LiveSoopId(p.channelUrl)).map(p=>Object.assign({_soopId:_b2LiveSoopId(p.channelUrl)},p)),univFiltered=_b2LiveUnivFilter==="\uC804\uCCB4"?soopPlayers:soopPlayers.filter(p=>String((p==null?void 0:p.univ)||"").trim()===_b2LiveUnivFilter),tierFiltered0=_b2LiveTierFilter==="\uC804\uCCB4"?univFiltered:univFiltered.filter(p=>p.tier===_b2LiveTierFilter),genderFiltered0=_b2LiveGenderFilter==="\uC804\uCCB4"?tierFiltered0:tierFiltered0.filter(p=>p.gender===_b2LiveGenderFilter),q=String(_b2LiveSearch||"").trim().toLowerCase(),searched=q?genderFiltered0.filter(p=>String(p.name||"").toLowerCase().includes(q)):genderFiltered0,tierOrder=typeof TIERS!="undefined"?TIERS:[],univOrder=typeof univCfg!="undefined"?univCfg.map(u=>u.name):[],_isLiveP=p=>{const st=_b2LiveStatusCache[p._soopId];return st&&st.live?0:1},tierFiltered=searched.slice().sort((a,b)=>{if(_b2LiveSortMode==="name"){const la2=_isLiveP(a),lb2=_isLiveP(b);return la2!==lb2?la2-lb2:String(a.name||"").localeCompare(String(b.name||""),"ko")}if(_b2LiveSortMode==="univ"){const ia=univOrder.indexOf(a.univ),ib=univOrder.indexOf(b.univ),ra=ia>=0?ia:999,rb=ib>=0?ib:999;if(ra!==rb)return ra-rb;const la2=_isLiveP(a),lb2=_isLiveP(b);return la2!==lb2?la2-lb2:String(a.name||"").localeCompare(String(b.name||""),"ko")}const idxA=tierOrder.indexOf(a.tier),idxB=tierOrder.indexOf(b.tier),rankA=idxA>=0?idxA:999,rankB=idxB>=0?idxB:999;if(rankA!==rankB)return rankA-rankB;const la=_isLiveP(a),lb=_isLiveP(b);return la!==lb?la-lb:String(a.name||"").localeCompare(String(b.name||""),"ko")}),liveFiltered=tierFiltered,sizeCfgMap={s:{min:70,avatar:34,stacked:!0,fullImage:!0,pad:"0",nameFs:"11px",tagFs:"7px",univLogoFs:"20px",gap:6},m:{min:132,avatar:50,stacked:!1,fullImage:!1,pad:"8px 9px",nameFs:"12px",tagFs:"9px",univLogoFs:"13px",gap:9}},sizeCfg=sizeCfgMap[_b2LiveCardSize]||sizeCfgMap.m,cardMinPx=sizeCfg.min;if(!soopPlayers.length)return`
      ...
      <div style="padding:60px 20px;text-align:center;color:var(--gray-l)">
        ...
        <div style="font-weight:700">\uD574\uB2F9 \uC870\uAC74\uC5D0 \uB9DE\uB294 \uC2A4\uD2B8\uB9AC\uBA38\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4</div>
         ...
         <span style="display:none;width:100%;height:100%;align-items:center;justify-content:center;background:${univColor}18;color:${univColor};font-size:18px;font-weight:1000">${safeNameHtml.slice(0,1)||"?"}</span>`:`<span style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:${univColor}18;color:${univColor};font-size:18px;font-weight:1000">${safeNameHtml.slice(0,1)||"?"}</span>`;let groupHeader="";if(showTierGroups){const tk=p.tier||"__none__";if(tk!==_lastTierKey){const isFirst=_lastTierKey===void 0;_lastTierKey=tk;const label=p.tier?typeof getTierLabel=="function"?getTierLabel(p.tier):p.tier:"\uBBF8\uC9C0\uC815";groupHeader=`
          ...
          </div>`}}const btnSize=sizeCfg.stacked?26:24,btnAccentStyle=p.univ?`border-color:${univColor}55;color:${univColor};background:${univColor}14;`:"",enlargeBtn=`<button type="button" class="b2-live-iconbtn" onclick="_b2LiveEnlarge('${p._soopId}','${safeName}')" title="\uD655\uB300\uBCF4\uAE30" aria-label="${safeNameHtml} \uD655\uB300\uBCF4\uAE30"
            style="width:${btnSize}px;height:${btnSize}px;border-radius:7px;font-size:${sizeCfg.stacked?12:10.5}px;line-height:1;padding:0;flex-shrink:0;${btnAccentStyle}">\u26F6</button>`,linkBtn=`<a href="https://ch.sooplive.co.kr/${p._soopId}" target="_blank" rel="noopener" class="b2-live-iconbtn"
            style="width:${btnSize}px;height:${btnSize}px;border-radius:7px;font-size:${sizeCfg.stacked?12:10.5}px;${btnAccentStyle}" title="SOOP \uCC44\uB110\uB85C \uC774\uB3D9" aria-label="${safeNameHtml} SOOP \uCC44\uB110\uB85C \uC774\uB3D9">\u{1F517}</a>`,avatarBlock=`
      <div style="position:relative;flex-shrink:0" onmouseenter="_b2LiveHoverEnter(this,'${p._soopId}','${safeName}')" onmouseleave="_b2LiveHoverLeave(event)">
        <button type="button" class="b2-live-avatar-btn${badgeInitDisplay==="inline-flex"?" is-live":""}" onclick="openPlayerModal&&openPlayerModal('${safeName}')" title="${badgeInitDisplay==="inline-flex"?"\uC120\uC218 \uC0C1\uC138 \xB7 \uB9C8\uC6B0\uC2A4 \uC62C\uB9AC\uBA74 \uB77C\uC774\uBE0C \uBBF8\uB9AC\uBCF4\uAE30":"\uC120\uC218 \uC0C1\uC138"}"
      ...
      </div>`,nameHtml=`<span class="b2-live-name" style="font-weight:900;font-size:${sizeCfg.nameFs};cursor:pointer;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--text)" onclick="openPlayerModal&&openPlayerModal('${safeName}')">${p.name||""}</span>`,univLogoSize=typeof getUnivLogoSizeStr=="function"?getUnivLogoSizeStr(p.univ,"players",sizeCfg.univLogoFs):sizeCfg.univLogoFs,univTag=p.univ?`<span class="b2-live-tag" title="${String(p.univ).replace(/"/g,"&quot;")}" style="display:inline-flex;align-items:center;font-size:${sizeCfg.tagFs};padding:2px 6px;background:${univColor}20;color:${univColor};border:1px solid ${univColor}3d;white-space:nowrap">${typeof gUI=="function"?gUI(p.univ,univLogoSize):""}</span>`:"",tierTag=p.tier?`<span class="b2-live-tag" style="font-size:${sizeCfg.tagFs};padding:1px 6px;background:${tierBg};color:${tierFg};white-space:nowrap;box-shadow:0 1px 3px rgba(0,0,0,.12)">${typeof getTierLabel=="function"?getTierLabel(p.tier):p.tier}</span>`:"",cardBg=p.univ?`${univColor}28`:"var(--white)",cardBody=sizeCfg.fullImage?`
        <div style="position:relative;width:100%;aspect-ratio:3/4;background:${univColor}12;overflow:hidden"
          ...
          <div style="position:absolute;left:0;right:0;bottom:0;padding:5px 6px;display:flex;flex-direction:column;gap:3px;z-index:1">
            ...
            <div style="display:flex;gap:3px;flex-wrap:wrap">
              ${p.univ?`<span class="b2-live-tag" title="${String(p.univ).replace(/"/g,"&quot;")}" style="display:inline-flex;align-items:center;font-size:${sizeCfg.tagFs};padding:2px 6px;background:rgba(255,255,255,.2);color:#fff;border:1px solid rgba(255,255,255,.32);white-space:nowrap;backdrop-filter:blur(2px)">${typeof gUI=="function"?gUI(p.univ,univLogoSize):""}</span>`:""}
            </div>
          </div>
        </div>`:`
