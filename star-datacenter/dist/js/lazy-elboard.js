...
function rElboard(C,T){if(T.innerText="\u{1F9FE} \uCD5C\uADFC\uC804\uC801",!isLoggedIn){C.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 20px;text-align:center;gap:16px">
    ...
    ${srcOpts.map(o=>`<button class="pill ${window._elSrc===o.id?"on":""}" style="flex-shrink:0;white-space:nowrap"
      ...
      <div class="empty-state-desc">\uACBD\uAE30 \uB370\uC774\uD130\uAC00 \uCD94\uAC00\uB418\uBA74 \uC5EC\uAE30\uC5D0 \uD45C\uC2DC\uB429\uB2C8\uB2E4</div></div>`,C.innerHTML=h;return}h+='<div id="elboard-list">';let lastDate=null;if(shown.forEach(({src,srcLabel,m,i,aLabel,bLabel,aColor,bColor})=>{const date=m.d||"\uB0A0\uC9DC\uBBF8\uC815";date!==lastDate&&(lastDate!==null&&(h+='<div style="height:4px"></div>'),h+=`<div style="padding:6px 0 4px;font-size:12px;font-weight:800;color:var(--text3);letter-spacing:.03em">${date}</div>`,lastDate=date);const sa=Number(m.sa),sb=Number(m.sb),aWin=sa>sb,bWin=sb>sa,isCK=src==="ck"||src==="pro"||src==="tt";let membersHtml="";if(isCK&&m.teamAMembers&&m.teamBMembers){const mA=(m.teamAMembers||[]).slice(0,4).map(p=>p.name).join(", "),mB=(m.teamBMembers||[]).slice(0,4).map(p=>p.name).join(", ");membersHtml=`<div style="font-size:11px;color:var(--gray-l);margin-top:4px;display:flex;gap:8px;flex-wrap:wrap">
      ...
      </div>`}else if(!isCK&&m.sets&&m.sets.length){const gameCount=m.sets.reduce((s,st)=>s+(st.games?st.games.length:st.playerA?1:0),0);gameCount&&(membersHtml=`<div style="font-size:11px;color:var(--gray-l);margin-top:3px">${gameCount}\uAC8C\uC784</div>`)}const subLabel=m.n?`<span style="font-size:10px;padding:2px 7px;border-radius:10px;background:var(--surface);border:1px solid var(--border);color:var(--text3)">${m.n}</span>`:m._label?`<span style="font-size:10px;padding:2px 7px;border-radius:10px;background:var(--surface);border:1px solid var(--border);color:var(--text3)">${m._label}</span>`:"",_mc={mini:"#7c3aed",univm:"#16a34a",ck:"#f59e0b",pro:"#0ea5e9",tt:"#10b981",all:"#2563eb"}[src]||"#64748b";h+=`<div class="rec-summary rec-mode-${src}" data-rec-mode="${src}" style="--rec-mode-col:${_mc};--rec-mode-rgb:${(hex=>{const h2=String(hex||"").replace("#","");if(h2.length!==6)return"100,116,139";const r=parseInt(h2.slice(0,2),16),g=parseInt(h2.slice(2,4),16),b=parseInt(h2.slice(4,6),16);return`${r},${g},${b}`})(_mc)};margin-bottom:6px">
      <div class="rec-sum-header" style="flex-wrap:wrap;gap:8px">
        ...
        <div class="rec-sum-vs" style="flex:1;min-width:200px">
          ...
          <span style="font-size:11px;font-weight:700;color:${aWin||bWin?"var(--win-col)":"#6b7280"}">
    ...
    </div>`}h+=`<div style="text-align:center;padding-top:4px;font-size:11px;color:var(--gray-l)">
