(function(){
  function _shareCardInitSlideTimers(card){
    // innerHTML에 삽입된 <script>는 브라우저에서 실행되지 않으므로
    // 렌더 후 이 함수를 호출해 슬라이드 타이머를 직접 시작한다
    try{
      const markers = (card||document.getElementById('share-card'))?.querySelectorAll?.('.sc-slide-init-marker');
      if(!markers||!markers.length)return;
      markers.forEach(marker=>{
        try{
          const key = marker.getAttribute('data-sc-slide-key');
          const total = parseInt(marker.getAttribute('data-sc-slide-total')||'1',10);
          if(!key||total<=1)return;
          window.__sharecardSlideTimers = window.__sharecardSlideTimers||{};
          if(window.__sharecardSlideTimers[key]) clearInterval(window.__sharecardSlideTimers[key]);
  ...
  function _shareCardPrefsSignature(){
    try{
      return JSON.stringify({
        ...
        losergray_default: localStorage.getItem('su_sc_losergray_default')||'inherit',
        'losergray_procomp-bkt': localStorage.getItem('su_sc_losergray_procomp-bkt')||'inherit',
        ...
        profile_default: localStorage.getItem('su_sc_profile_pct_default')||'inherit',
        'profile_procomp-bkt': localStorage.getItem('su_sc_profile_pct_procomp-bkt')||'inherit',
        ...
        font_default: localStorage.getItem('su_sc_font_pct_default')||'inherit',
        'font_procomp-bkt': localStorage.getItem('su_sc_font_pct_procomp-bkt')||'inherit',
        ...
        cardshape_comp: localStorage.getItem('su_sc_cardshape_comp')||'inherit',
        cardshape_default: localStorage.getItem('su_sc_cardshape_default')||'inherit',
        'cardshape_procomp-bkt': localStorage.getItem('su_sc_cardshape_procomp-bkt')||'inherit',
