...
function openUnivModal(univName){
  ...
  const sub=document.getElementById('univModalSub');
  if(sub){
    sub.innerText=`소속 ${members.length}명 · 대표 색상 ${u.color||'미설정'} · 전적과 구성원 정보 확인`;
  }
  const heroMeta=document.getElementById('univModalHeroMeta');
  if(heroMeta){
    const wins=members.reduce((s,p)=>s+(p.win||0),0);
    const losses=members.reduce((s,p)=>s+(p.loss||0),0);
    const pts=members.reduce((s,p)=>s+(p.points||0),0);
    const tot=wins+losses;
    const wr=tot?Math.round(wins/tot*100):0;
    heroMeta.innerHTML = `
      <span class="detail-hero-chip"><b>${members.length}</b><small>인원</small></span>
      <span class="detail-hero-chip"><b>${tot?wr+'%':'-'}</b><small>승률</small></span>
      <span class="detail-hero-chip"><b>${pts>=0?'+':''}${pts}</b><small>포인트</small></span>
    `;
  }
  const avatar=document.getElementById('univModalAvatar');
  if(avatar){
    const url=UNIV_ICONS[univName]||(univCfg.find(x=>x.name===univName)||{}).icon||'';
    avatar.innerHTML = url
      ? `<img src="${toHttpsUrl(url)}" alt="" style="width:42px;height:42px;object-fit:contain" onerror="this.style.display='none';this.parentElement.innerHTML='<div class=&quot;detail-avatar-fallback detail-avatar-fallback--univ&quot;>${String(univName||'?').slice(0,1)}</div>'">`
      : `<div class="detail-avatar-fallback detail-avatar-fallback--univ">${String(univName||'?').slice(0,1)}</div>`;
  }
  ...
  st.editOpen=false;
  try{
    window.__suModalUnivName = univName;
    window.__suModalPlayerName = '';
    if(typeof window.__suSyncUrlState==='function') window.__suSyncUrlState();
  }catch(e){}
  om('univModal');
  const editBtn=document.getElementById('univEditBtn');
  const editBtnMobile=document.getElementById('univEditBtnMobile');
  if(editBtn) editBtn.style.display=isLoggedIn?'inline-flex':'none';
  if(editBtnMobile) editBtnMobile.style.display=isLoggedIn?'inline-flex':'none';
  requestAnimationFrame(()=>{
    const btn=document.getElementById('univEditBtn');
    if(btn) btn.style.display=isLoggedIn?'inline-flex':'none';
    const btnM=document.getElementById('univEditBtnMobile');
    if(btnM) btnM.style.display=isLoggedIn?'inline-flex':'none';
  });
}
...
function toggleUnivEdit(){
  ...
  const btn=document.getElementById('univEditBtn');
  const btnMobile=document.getElementById('univEditBtnMobile');
  if(st.editOpen){
    if(btn) btn.textContent='✕ 닫기';
    if(btnMobile) btnMobile.textContent='✕ 닫기';
    ...
    if(btnMobile) btnMobile.textContent='✏️ 수정';
