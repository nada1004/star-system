/* ══════════════════════════════════════
   Render Share Match Openers
══════════════════════════════════════ */
async function openCompMatchShareCard(tnId, gi, mi){
  const ok = await _shareEnsureStatsAndOpen();
  if(!ok) return;
  try{
    if(typeof window.__openCompMatchShareCardImpl === 'function'){
      return window.__openCompMatchShareCardImpl(tnId, gi, mi);
    }
    if(typeof window.openCompMatchShareCard === 'function' && window.openCompMatchShareCard !== openCompMatchShareCard){
      return window.openCompMatchShareCard(tnId, gi, mi);
    }
  }catch(e){}
}
...
function openShareCardFromMatch(mode,idx){
  const arr=mode==='mini'?miniM
    ...
    :mode==='gj'?(typeof gjM!=='undefined'?(gjM||[]).filter(x=>!x._proLabel):[])
    :mode==='progj'?(typeof gjM!=='undefined'?(gjM||[]).filter(x=>!!x._proLabel):[])
    :mode==='tt'?(typeof ttM!=='undefined'?ttM:[])
    :miniM;
  const m=arr[idx]||null;
  const isSolo=(mode==='ind'||mode==='gj'||mode==='progj');
  ...
  const rawShareObj=m?(isSolo?{
    ...
    ...m,
    _matchType:isCKorPro?mode:(isTier?'tt':(m._matchType||mode||'')),
  ...
  const shareObj = (typeof window.HistoryActionUtils?.normalizeMatchShareObjForCard === 'function')
    ? window.HistoryActionUtils.normalizeMatchShareObjForCard(rawShareObj, mode)
    : rawShareObj;
  if(typeof window.HistoryActionUtils?.openUnifiedMatchShareCard === 'function'){
    window.HistoryActionUtils.openUnifiedMatchShareCard(shareObj);
    return;
  }
