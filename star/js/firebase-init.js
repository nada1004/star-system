...
async function _pushFirebaseSignal(meta){
  const pw = (typeof suGetSecret==='function' ? suGetSecret('su_fb_pw') : (localStorage.getItem('su_fb_pw')||'')) || FB_AUX_DEFAULT_PW;
...
// 초기 로드 + 변경 신호 기반 가져오기
// - 첫 진입 시 보조 신호만 확인
// - 이후에도 보조 신호(다른 기기 변경) 또는 수동 동기화 때만 GitHub 확인
setTimeout(()=>{ _pollFirebaseSignalOnce(true); }, 150);
setInterval(()=>{
