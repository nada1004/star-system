...
CSS 8개 파일(각각 별도 &lt;link&gt; 요청, 전부 render-blocking) → **1개 번들**로 통합
...
| `dist/css/bundle.css` | style.css 등 8개 CSS 파일 (순서 그대로 concat, 캐스케이드 유지) |
...
CSS 번들의 `?v=` 값은 내용 기반 MD5 해시(앞 10자리)라서, CSS 내용이 안 바뀌면
재빌드해도 버전 값이 그대로 유지되어 재방문자가 불필요하게 다시 받지 않습니다.

### 2. Minification (esbuild)
- 전체 JS **5.0MB → 4.1MB (-18%)**
- CSS 번들 **839.7KB → 638.1KB (-24%)**
- 공백, 주석, 불필요한 코드 제거
...
| 시나리오 | 이전 | 이후 (예상) |
|----------|------|-------------|
| 최초 방문 HTTP 요청 수 | ~148개 (JS 140 + CSS 8) | 12개 (초기 6개: JS 5 + CSS 1) |
| 최초 방문 JS+CSS 전송량 | 6.22MB | 5.44MB (-12.5%) |
